...
1 — WHAT SHIPPED. Detector second pass with role-neutral pairs `{sourceUid, targetUid, kind}` + the aura pass (eligibility inversion: the source's EFFECT determines targets, the target needs no triggers) — this is the ratified CF-60 glow-coverage fix (mana-potion, stamina-tonic, berserkers-greataxe). Client-side buff replica composing the pure detector + canonical getItem + sim-exported applyPct via deep-subpath import; adjacency DELTA only, sideStats never folded in. Three-class reveal hosted as an ADJACENCY section in ItemInfoPopover; affected-cell chips; mount gating default-OFF with the RoundResolution/S2b mount proven to render zero reveal surfaces over a LIVE synergy bag.

2 — RATIFIED DISPLAY RULINGS (all three verbatim in the PR body).
(a) RULE ABOVE, LIVE CONSEQUENCE BELOW. describeItem owns the rule statement. The ADJACENCY section renders ONLY what is live in this bag layout now; it never restates the rule.
(b) GATE-CLOSED EMPTY STATE KEPT — prior suppression lean REVERSED. "No adjacent items affected" is ACTIONABLE (it tells the player the item is doing nothing and invites a fix) and it AGREES with the dark glow rather than contradicting it. The defect was never the empty state; it was the duplicate above it. The held suppress-gate-closed candidate is resolved AGAINST suppression.
(c) CLASS-1 AMENDMENT — UNCONDITIONALLY LIVE. Class 1 requires the buff to be unconditionally live for the whole combat: either (i) applied at combat start (on_round_start aura), or (ii) a reaction buff with at least one provoker whose top-level trigger fires unconditionally (on_round_start / on_cooldown). A reaction buff whose every provoker is conditional (on_low_health-only) inherits class 2 — no after-value, zero totals, zero chips. GENERATIVE MECHANISM: conditionality PROPAGATES from provoker to buff; a display class asserted from relationship-EXISTENCE rather than firing-CERTAINTY produces a false guarantee. Supersedes the original § 2 class-1 wording ("deterministic from the provoker's first fire") by insertion; no rewrite.

3 — CATCH 87 + DRIFT 68 (Class A, master-dev, gate-surfaced). A PR-body edit template asserted "All [N] existing consumers … are unchanged at the call site" while deferring N to a grep not yet run — the grammar presupposed one branch of an explicitly deferred fact. N resolved to 1, and that consumer was the ratified HOST, modified by design; filling the template verbatim would have landed a claim contradicting the PR's own diffstat. Surfaced by Claude Code's Step-3 grep before POST. Counts per the gate-surfaced-AND-defect-capable distinguisher.

4 — CATCH 88 + DRIFT 67 (Class A, master-dev, playtest-surfaced). PR-A specified a new ADJACENCY section without specifying its relationship to describeItem, which already renders on_adjacent_trigger effects — so every card rendered the rule sentence byte-identically twice, ~40px apart, burying the affected rows beneath a repeat. The evidence was available and unapplied at authoring time: describeItem renders every trigger/effect — including on_adjacent_trigger — as a line, greppable in shipped source then and now, and the CF-89 content-texture inventory (ratified at decision-log.md 2026-07-22 § "CF-89 fork ROUTED …") had already recorded exactly that. Master-dev specified the new section without checking the renderer it would duplicate. Shipped to a build; surfaced only by Trey's playtest. Fixed round 2 (`3698b6a`) under ruling 2(a).

5 — DISTINGUISHER RESTATED (why Codex findings are +0 and these two are not). CF-67 holds: a Codex finding dispositioned before merge is the review WORKING — review yield, not a catch. An authoring defect that survives into a build or a handoff artifact is a catch regardless of who surfaces it. Both classes appear in this arc and the counter treats them differently by design.

6 — CODEX TALLY, +0 CATCHES. 4 rounds: R1 P2 (class-3 rows emitted without a provoker gate — a false RELATIONSHIP) fixed `5cece79`; R2 clean; R3 P2 (conditional-only provokers classed as deterministic — a false GUARANTEE) fixed `7df379a`; R4 clean, stale-SHA guard PASS. Raw findings 2, ceiling 2/4 never tripped, 0 meta-audits. Both source-confirmed before trust; both surfaced by Claude Code for authorization rather than self-applied.

7 — PATTERN 10 INSTANCE (referenced, NO new ordinal). The R1/R3 pair is a Pattern 10 case: the original class-1 rule was scoped to its first surface ("deterministic from the provoker's first fire") rather than its generative mechanism, so the next instance escaped it. The remedy was applied PROSPECTIVELY to ruling 2(c) — mechanism stated, then tested against a same-mechanism/different-surface variant (the on_round_start aura path), which drove the aura-path unification audit in the same round.

8 — GATES AT MERGE. 89/89 affected tests. Rule 28 red-then-green by ISOLATED REVERT for the original build and every fix round (round 2's falsifier asserts an occurrence COUNT of 1, not presence — presence-only passes on the defective tree). `tsc -b --noEmit --force` exit 0 on BOTH changed packages (turbo --force does not bust tsc -b's incremental cache). Rule 13 triple-green cold. Rule 19 rendered proof across all display states at 1280×720 and 390-wide.

9 — CANDIDATES HELD, NONE OPENED (measured-not-built; the board does not grow at a closing entry).
(a) Glow saturation at item density — at 11 items nearly the whole bag glows teal; PR-A's aura coverage increased it. CF-60's persistent glow predates reveal-on-intent; whether it should survive it is a real question. Strongest candidate for its own CF.
(b) Shop-card affordance — the R1 playtest bought a Whetstone with no weapon to sharpen and lost 0-dmg-dealt. The "why is this arrangement good" question fires at PURCHASE, not only at placement; the shop card has no such affordance. Likely reshapes PR-B scope.
(c) Class-2 "if triggered" copy ambiguity — reads identically for a conditional PROVOKER and a conditional HOST. Reported by Claude Code, deliberately unfixed (copy change mid-round is scope creep).
(d) Empty-state disambiguation — "no adjacent items pass matchTags" vs "adjacent items pass but none can provoke" share one string; only vampire-fang exercises the second case among weapons.
(e) Inert-buff row — a damage buff renders on an item with no damage effect (vampire-fang). Ratified affected-set rule is correct and unchanged; whether an inert row should render is a display question.
(f) Class-1/2 reveal at the S2b mount — would serve the 5-run note's clause 2; gated OFF deliberately.
(g) `bag_arrangement_snapshot` telemetry — no schema variant; wiring it is an additive bump (CF-41/CF-90 boundary).
(h) turbo run-1 load flake — watch item from round 2.
(i) Marauder invisibility — the class lever (+1 every damage effect) has no player-facing surface anywhere; the card correctly shows the adjacency delta only. L4 / CF-86 evidence; deferral cost now OBSERVED in real play, not inferred.

10 — STANDING WATCH, unresolved. The unminted workspace-source-subpath-import pattern candidate (first instance Phase 2.5i Path B; trigger: "watch in M2 for any new main-chunk file importing sim/content/ui-kit at static-link tier") is TRIPPED by `adjacencyReveal.ts` importing `@packbreaker/sim/src/math` — second instance. Codification deferred to the next arc that touches the boundary; next-free pattern ordinal is 11.

11 — SHARED-CONTRACT CHANGE. ui-kit Popover gained opt-in `presentation: 'anchored' | 'sheet'`, default anchored. Exactly ONE direct consumer (ItemInfoPopover, the ratified host, modified by design). Inside tech-architecture.md § 3's boundaries — prop type is ui-kit-owned, package still imports React only, nothing new crosses a package line, no § 3 `shared` exception required.

Counter: catches +2 (Catch 87, Catch 88) / rules +0 / patterns +0 (Pattern 10 referenced, not minted) / drifts +2 (Drift 67, Drift 68) / open-CFs +0 (CF-89 stays OPEN; all nine § 9 candidates HELD, none opened; the § 10 standing watch remains unresolved and unopened — a watch is not a candidate and is not counted as one). Running line 86/34/10/66/53 → 88/34/10/68/53.

---

## 2026-07-22 — CF-89 L1/L2 PHASE 1 RATIFIED: scope-fork verdict CLIENT-ONLY / resolved-numbers ratified (option (i) client replica; option (ii) sim-helper extraction REJECTED); three-display-class taxonomy ratified (locked visual covers class 1 only — classes 2–3 ship as text variants in the locked shell, Design deviation on record, no re-round); RoundResolution readOnly mount GATED OFF (enabling-there held, not opened); Phase 2 SPLIT into PR-A (L1) + PR-B (L2); ItemInfoPopover host-vs-sibling deferred to a Rule-8 Step-0 inspection gate; two Step-0 verification riders (effect-level buff_adjacent matchTags / canonical-copy sync); counter 86/34/10/66/53 → 86/34/10/66/53

Docs-only, insertion-only. Baseline tip f09d9c8. Ratifies the CF-89 L1/L2 Phase 1 read-only map (zero mutations, tree = session-start) and six master-dev amendments to it. No code, schema, content, or corpus change. CF-89 stays OPEN.

1 — SCOPE FORK RATIFIED: client-only, resolved numbers. No shared pure buff-resolution function exists in packages/sim (sumActiveBuffs module-private + consumes a mid-combat runtime type; computeDamageStats is post-combat per-side; deriveSideStats/computeAdjacents private). Option (i) CLIENT REPLICA ratified: the client composes its existing pure detector + canonical getItem + the sim-barrel-exported applyPct, replicating four verified rules (additive stacking with (source,target,stat) dedupe; all shipped buff_adjacent effects omit durationTicks → full-combat; a damage buff keys on the firing item; applyPct flooring). Option (ii) sim-side helper extraction REJECTED — adds packages/sim export surface for no gain. Zero sim/schema/corpus surface touched.

2 — THREE-DISPLAY-CLASS TAXONOMY RATIFIED. Class 1 DETERMINISTIC (on_round_start auras from tick 0; on_adjacent_trigger buffs from the provoker's first fire) → resolved integer-floored after-value (e.g. Whetstone +1 on Iron Sword damage 4 → 5; Mana Potion −15 on Iron Sword cooldown 50t → 42t → "4.2s"). Class 2 CONDITIONAL/PROBABILISTIC (on_low_health auras — berserkers-greataxe; trigger_chance_pct — rune-pedestal, master-alchemists-kit) → label only ("if triggered" / chance), NEVER a flat after-value. Class 3 OPPONENT-STATUS REACTION (spark-stone, fire-oil: on_adjacent_trigger → apply_status to opponent) → condition + effect, NO affected-items panel (no adjacent item is affected). CORRECTNESS RULING: for a reaction buff the affected set is the effect-matchTags-filtered ADJACENT set gated on at least one provoker existing — NOT the provoker set. The locked mockup conflates them (its example item is both); the implementation must not.

3 — DESIGN DEVIATION ON RECORD, no re-round. The Claude Design artifact locks class 1 only. Classes 2–3 ship as text / section-visibility variants inside the SAME locked card shell (suppress the after-value row; swap the condition line; suppress the affects panel for class 3). Shell, palette, anchor logic, and mobile bottom-sheet solution are unchanged and remain locked.

4 — RoundResolution readOnly mount GATED OFF. BagBoard is mounted at DesktopRunScreen, MobileRunScreen, and RoundResolution (the CF-85 S2b readOnly opponent-build reveal). L1/L2 additions inside BagBoard would inherit into S2b; they are GATED OFF there by explicit prop. Enabling them at the S2b mount would serve the 5-run note's clause 2 ("what the ghost uses against me") — HELD candidate, NOT opened (measured-not-built).

5 — PHASE 2 SPLITS: PR-A (L1) then PR-B (L2). PR-A = adjacency.ts second detection pass with inverted aura roles (NOT a predicate widening — the aura filter is effect-level, the current predicate reads trigger-level) + a kind/role field on the pair shape + the class-1/2/3 reveal + affected-cell chips + the CF-60 glow-coverage fix (mana-potion, stamina-tonic, berserkers-greataxe). PR-B = the recipe reverse index (built, none exists) + owned/not-owned input derivation on the scoutRecipes multiset pattern + the "combines into →" surface. Sequenced A then B; CF-85 PR-A/PR-B precedent.

6 — ItemInfoPopover HOST-vs-SIBLING → Rule-8 Step-0 gate. CF-57's per-item inspect popover already ships on bag + shop and already renders describeItem trigger text; two popovers on one tile is a defect. PREFERRED: host the locked card as a section inside ItemInfoPopover. FALLBACK: a sibling reveal at BagBoard's shared mount. PR-A opens with a read-only inspection of ItemInfoPopover's shell + mobile behavior and HALTS-AND-SURFACES if hosting is refuted — the disposition is the inspection's, not asserted here.

7 — TWO STEP-0 VERIFICATION RIDERS. (a) Confirm the effect-level matchTags field on the buff_adjacent Effect variant in content-schemas.ts AND its packages/content mirror (byte-identical per the schema-sync mechanism). (b) The CANONICAL project copy of content-schemas.ts shows buff_adjacent as {type, stat, amount, durationTicks?} with NO matchTags, while the Phase 1 report cites one; balance-bible.md corroborates the repo (it distinguishes "weapon adjacents only" from "all adjacents", unimplementable without effect-level scoping). If the repo has the field, the canonical project copy is STALE — a canon-sync defect on a source-of-truth file, to be dispositioned at that Step 0. UNDISPOSITIONED HERE; NO catch minted on a likelihood (Rule 30 posture).

Counter: catches +0 / rules +0 / patterns +0 / drifts +0 / open-CFs +0 (CF-89 stays OPEN; the § 4 enabling-candidate is HELD, not opened). Running line 86/34/10/66/53 → 86/34/10/66/53. Rule 20: no PR-A or PR-B implementation prompt cites this entry until it carries a real SHA.

---

## 2026-07-22 — CF-89 fork ROUTED: content-texture inventory verdict (a) SURFACE ratified; CF-89 M2 character flips content-design → legibility-led (reopens toward UI); (b) counter-texture residue logged; § 3 "nothing to surface" superseded (no catch — committed-disposition revised by commissioned follow-up, not drift); Catch 86 + Drift 66 (master-dev ambiguous-citation drafting defect, gate-caught by decision-log-close); WATCH (CF-91-OPENED § 4) RESOLVED on this counter-moving instance; CF-60 glow-gap + adjacency.ts stale-comment held as candidates; counter 85/34/10/65/53 → 86/34/10/66/53

Docs-only, insertion-only. Baseline tip 9df7673. Routes the CF-89 fork on the read-only content-texture inventory (zero mutations, tree = session-start). No code/schema/content/corpus change.

1 — VERDICT (a) SURFACE ratified. Synergy/counter texture exists but is invisible. Forcing evidence: the reactive-heal ⟳ small-hits ⟳ DoT counter-triangle is sim-real + bible-named (balance-bible.md § 4 burn "Bypasses on_taken_damage triggers" / poison "Ignores armor heals"; § 6 wooden-shield "Strong vs many small hits") with zero player-facing surface — a legibility gap, not a content gap. (c) rejected on unused shape headroom (3/6 triggers provoke; summon_temp_item + passiveStats.bonusBaseDamage zero-item). (b) named as residue (counter-variety thinness, Marauder adjacency-starvation, Tinker recipeBonusPct floors +0 on 10/12 outputs unstacked — mostly CF-86/backlog), NOT folded into CF-89. CF-89 stays OPEN, DEFERRED M2, character = legibility-led/UI-inclusive.

2 — § 3 SUPERSEDED, no catch. SUPERSEDES (insertion-only, no rewrite) the 2026-07-21 § "CF-85 ARC DISPOSITION + Q3 COUNTER-SYSTEM VERDICT (a) …" § 3 line "no hidden counter logic for a UI to surface / M2 = content-design not a Claude Code prompt": the § 2 trigger-economy counter IS surfaceable. A committed forward-disposition revised by its own commissioned follow-up (this inventory) is NOT a claim-accuracy drift — distinct from Catches 82-84 (false shipped-state assertions, wrong-on-landing). Its own category; no catch.

3 — Catch 86 (Class A, citation-discipline) + Drift 66. The draft of THIS entry cited the WATCH as bare "2026-07-21 § 4" — ambiguous across three same-day § 4s, violating the date+header citation invariant; it also prematurely declared that WATCH "resolved" on the then-counter-neutral draft, against the WATCH's codify-on-counter-moving trigger (same careless-WATCH-clause root, bundled). Surfaced in-gate by decision-log-close (halt-and-surface) before the append landed. Per Catches 82-84 (master-dev authoring defect surfaced by a verification gate → counted; distinct from CF-67 Codex-review yield, which is the review working, not a catch): Catch 86, Drift 66 (paired). Fixed in this landed text (full-header citation; correct resolution below).

4 — WATCH RESOLVED (this is the counter-moving instance its trigger required). Distinguisher codified: a master-dev drafting error SELF-surfaced by the author before any check (pre-propagation self-catch, per the item-5 exclusion at 2026-07-21 § "CF-85 ARC DISPOSITION + Q3 COUNTER-SYSTEM VERDICT (a) …" and prior) is EXCLUDED; a drafting error surfaced by a verification gate / other actor (handoff-verify, Step-0, decision-log-close), even before landing, is COUNTED (Catches 82-84, now 86). Extends the pre-propagation-self-catch-exclusion convention — no rule ordinal. The WATCH at decision-log.md 2026-07-21 § "CF-91 OPENED: run-end per-round strip renders draw as loss …" § 4 is CLOSED.

5 — Candidates HELD, not opened (measured-not-built): CF-60 glow-coverage gap (3 static-aura buff_adjacent items — mana-potion, stamina-tonic, berserkers-greataxe — unglowed; detector scans on_adjacent_trigger only); adjacency.ts header comment stale line-ref (558-589 → 681-712, semantics intact; ride-along).

Counter: catches +1 (Catch 86) / rules +0 (convention extension) / patterns +0 / drifts +1 (Drift 66) / open-CFs +0. Running line 85/34/10/65/53 → 86/34/10/66/53. Rule 20: no CF-89 design/build prompt cites this entry until it carries a real SHA.
...
### 1 — CF-91 OPENED: run-end draw-display gap (new CF, completes CF-84's honest-draw surface)
The Step-0 read-only halt (this session, zero mutation, tip `9852fa9`) proved RunEndScreen's per-round "Per-round breakdown" strip renders a draw round as a red **L**, structurally: a draw is collapsed to `'loss'` at `packages/sim/src/run/state.ts:1037` (`const roundOutcome: RoundOutcome = input.outcome === 'player_win' ? 'win' : 'loss'`) BEFORE the history entry is written (`state.ts:1056-1064`, `outcome: roundOutcome`), and `RunEndScreen.tsx:202-205` reads that already-collapsed `RoundOutcome` (`Map<number,'win'|'loss'>`), so the pip (`RunEndScreen.tsx:113-139`, renders `W`/`L`/`·`) has no draw value to branch on. The un-collapsed `CombatOutcome` survives only on the fire-and-forget `combat_end` telemetry (`state.ts:1074`), never retained in run state. Fix is **schema + sim**; client-only REFUTED (no client edit recovers data the sim already discarded). Ordinal walk (live grep, shown not asserted): highest CF token **CF-90**; `CF-91`-or-higher → **0**; next free → **CF-91** OPENED. **New CF, NOT a CF-84 reopen** — the CF-88 precedent (decision-log.md 2026-07-20 § "CF-83 RAMP + CF-84 DRAW SEMANTICS CLOSED …" item 6: a gap EXPOSED BY but not COVERED BY CF-84's closed scope opens its own ordinal; closed-stays-closed, no implicit reopen). CF-91 completes the honest-draw surface CF-84 began. OPEN.

### 2 — Scope (B1) RATIFIED: additive `combatOutcome` on RunHistoryEntry
`RoundOutcome` stays `'win' | 'loss'` — the `draw → 'loss'` map at `state.ts:1037` is UNCHANGED, every economic consumer stays byte-untouched (heart decrement `state.ts:1053-1054`, `trophyDeltaFor` `ruleset.ts:137`, run-end detection `state.ts:1274-1278`, `round_end` telemetry `state.ts:1086`), and CF-84's ratified heart/trophy 2-way cost ("draw still costs") is preserved BY CONSTRUCTION — the economic type never learns about draw. The fix RETAINS the full `CombatOutcome` as an additive `combatOutcome` SOURCE field on `RunHistoryEntry` — the additive-source-field precedent (CF-15 `opponentClassId`; CF-43 `bornFromRecipe`: a source value carried on the history entry without transforming the economic path). RunEndScreen's pip reads `combatOutcome` → renders a distinct **D**, reusing CF-84's overlay DRAW label/styling (no new visual token; Gridline locked per visual-direction.md).

**(A) widen `RoundOutcome` to `win|loss|draw` — CONSIDERED and REJECTED.** (A) spreads a DISPLAY concern into the ECONOMIC type: it forces a `draw` mapping at every economic site (trophy, hearts, run-end, telemetry) plus a `.json` scenario-fixture re-baseline, and the economic 2-way (draw costs like a loss) IS CF-84's ratified semantics (decision-log.md 2026-07-19 § "CF-83 RAMP + CF-84 DRAW SEMANTICS RATIFIED …": "4th-outcome-member REJECTED"). Noted tradeoff: (A) buys source-level honesty (one outcome type, no parallel field) at the cost of blast radius + re-ratifying a settled economic semantics; (B1) factors the display concern additively, leaving the economic type at its ratified 2-way. (B1) wins on the additive-factoring precedent.

### 3 — Catch-vs-followup fork: RESOLVED +0 (CF-84 close text is overlay-scoped; no over-claim)
Dispositioned against CF-84's ACTUAL close text (decision-log.md 2026-07-20 § "CF-83 RAMP + CF-84 DRAW SEMANTICS CLOSED …"), quoted verbatim: item 1 — **"CF-84 render shipped: RoundResolution is 3-way (a draw renders DRAW, not the pre-CF-84 'LOST'/'DEFEAT')"**; items 2 + 8 — the Round-3 KO-flash fix extracted **`koFlashTargets(outcome)` … `draw → {player, ghost}`** on `CombatScene`. The render claim is SCOPED to the resolution overlay (RoundResolution) + the combat scene's KO flash; it makes NO all-surfaces / global honest-draw-rendering claim and never asserts RunEndScreen renders draw. So NO false claim sits in canon → the master-dev's +0 lean is VERIFIED. **NO catch minted (the next free catch-ordinal-86 stays free), NO paired drift (drift-ordinal-66 not minted).** CF-91 is a CLEAN followup surface CF-84 legitimately did not scope, not a close-completeness gap. Had the close instead read "all surfaces render DRAW", this would have fired a Class C1 close-completeness catch + a paired drift (the close-over-claim branch, running line 86/34/10/66/54); it did not.

### 4 — WATCH (no ordinal): pre-propagation-self-catch exclusion vs the counted-in-gate class
Carried from decision-log.md 2026-07-21 § "CF-85 ARC DISPOSITION + Q3 COUNTER-SYSTEM VERDICT (a) …" item 5's correction: the boundary between the pre-propagation-self-catch exclusion (an actor's own in-flight drafting error caught before it enters the log → excluded) and the counted master-dev claim-accuracy-caught-in-gate class (Catches 82/83/84 → counted) is stated by example but not yet reduced to a single stated distinguisher. Recorded as a WATCH; codify the distinguisher on the next instance that MOVES a counter on this axis (not on this docs-only no-counter-move entry). No ordinal.

### Counter
Light entry (CF open + scope ratification; no PR, no merge). Ordinal walk (live from canon; tip `9852fa9` = decision-log.md 2026-07-21 § "CF-85 ARC DISPOSITION + Q3 COUNTER-SYSTEM VERDICT (a) …" carrying **85/34/10/65/53**): highest **Catch 85**, **Rule 34**, **Pattern 10**, **Drift 65**, **CF-90** (grep-confirmed this session). Deltas by ID: catches **+0** (step-3 fork resolved +0 — CF-84 close text overlay-scoped, no over-claim); rules **+0**; patterns **+0**; drifts **+0** (item 4 a watch, no ordinal); open-CFs **+1** (CF-91 OPENED; NONE closed — CF-84 stays CLOSED, not reopened). Running line: **85/34/10/65/53 → 85/34/10/65/54** — catches **85** / rules **34** / patterns **10** / drifts **65** / open-CFs **54**. Docs-only; the counter anchor is this entry's own docs commit. (The rejected close-over-claim branch would have read 86/34/10/66/54 — it did NOT fire; see § 3.)

Open-CF touch (delta only; no other state change):
- **CF-91** — run-end per-round strip renders draw as loss (RunEndScreen). OPENED (this entry), scope (B1) ratified: additive `combatOutcome` on `RunHistoryEntry`, schema + sim. Completes CF-84's overlay-scoped honest-draw render. OPEN — implementation gated by Rule 20 (needs this entry's SHA).

---

## 2026-07-21 — CF-85 ARC DISPOSITION + Q3 COUNTER-SYSTEM VERDICT (a): no mitigation/penetration system (defense = flat HP + heals, offense = flat additive damage); the CF-89-deferral premise VERIFIED TRUE by read-only inspection; trigger-economy counter carried as M2 design input (opens NO CF); CF-89 stays OPEN/deferred; counter UNCHANGED 85/34/10/65/53

Docs-only, insertion-only. Baseline: tip `61d04c7` (decision-log.md 2026-07-21 § "`replay_viewed` telemetry CF OPENED at CF-90 …") carrying **85/34/10/65/53**. Lands the CF-85 legibility-arc disposition that lived in chat pending the Q3 counter-system read-only check, per the standing precedent that arc dispositions land in their own ratified entry (decision-log.md 2026-07-19 § "CF-83 FIX DIRECTION CORRECTED …"). No code, schema, corpus, migration, or CF-state change; opens NO CF, closes NO CF.

### 1 — Q3 counter-system verdict (a): NO system; the CF-89-deferral premise is VERIFIED TRUE
Read-only inspection this session (zero mutations; working tree showed only the pre-existing untracked `scratch/`, tip unchanged `61d04c7`). Verdict **(a) no counter system**: defense is flat HP + heals, offense is flat additive damage, and no mitigation / penetration / damage-reduction stat exists in schema, content, or the bible. Forcing evidence:
- **Damage → HP is direct subtraction, no mitigation term.** `combat.ts:485` — `target.hp = Math.max(0, target.hp - pd.rawAmount)` — the only target-side quantity read is the target's own current HP (the overkill clamp on the logged amount at `:484`). The amount is computed offense-side only at `combat.ts:733` — `const finalAmount = baseAmount + buffSum + state.sideStats[sourceSide].bonusBaseDamage` — no defender field enters. The other two HP-mutation paths (status DoT `combat.ts:571`/`:582`, the CF-83 ramp `combat.ts:334-338`) are equally flat.
- **No defensive stat in schema.** `Item` (content-schemas.ts § 3), `PassiveStats` (`maxHpBonus` / `bonusBaseDamage` / `goldPerRound`), `BuffableStat` (`damage` / `cooldown_pct` / `trigger_chance_pct`), and `RelicModifiers` carry no armor / block / resist / shield / absorb field. `armor` exists only as an `ItemTag` (a `matchTags` label), never read in damage math.
- **No penetration / true-damage stat** in schema or content (grep of `packages/content/src` + `packages/sim/src` for `penetrat|pierce|true damage|armor-ignore` → 0 substantive hits). All three statuses are pure DoT/control (`status.ts`): burn/poison per-second stack DoT, stun skips one `on_cooldown` — none defensive.
- **Bible specs = wires the no-mitigation model.** `balance-bible.md:47` — "Player base damage | 0 | All damage comes from items unless modified by class/relic"; `balance-bible.md:145` — "**Armor (4)** — modeled via heals + low-HP triggers". The § 17 schema-gap list and § 18 open levers anticipate NO defensive stat. Nearest Pattern-9 shape ruled out: `Item.passiveStats.bonusBaseDamage` is schema-present (content-schemas.ts:210), populated by zero items, bible-flagged "reserved for future use (deferred to M2)" (`balance-bible.md:376`) — but it is flat damage, not counter-play, so it does not force verdict (c).

The CF-89-deferral premise (M1 combat depth is thin; no counter system for a UI to surface) was Trey's stated UNCERTAINTY at the redraw; it is now EVIDENCE-BACKED and VERIFIED TRUE.

### 2 — Trigger-economy caveat — M2 design INPUT, not a stat system (opens NO CF)
One counter-shaped interaction exists at the TRIGGER-ECONOMY level, not the stat level: reactive heals (`on_taken_damage: heal`) counter many-small-hits, and DoT — non-reactive by design (`combat.ts:562-564`) — counters reactive heals. Bible-named: `balance-bible.md:103` burn "Bypasses `on_taken_damage` triggers", `:104` poison "Ignores armor heals", `:146` Wooden Shield "Strong vs many small hits". This does NOT flip the verdict — no stat reduces damage; every hit lands at full value. Carried as an INPUT that reframes the M2 content-design conversation (the depth that exists is timing / trigger-economy, not stat interplay). Per the out-of-scope constraint it opens NO CF; scoping any CF off it is forbidden ahead of the M2 design decision.

### 3 — CF-89 disposition CONFIRMED: stays OPEN, deferred to M2, now evidence-backed
CF-89 (the "why is this arrangement good" choosing/why affordance, opened at decision-log.md 2026-07-20 § "CF-85 SCOPE REDRAWN against Phase-1 read-only …") stays OPEN and DEFERRED to M2. The deferral rationale is no longer an uncertainty: the read-only inspection (§ 1) confirms there is no hidden counter logic for a UI to surface, so the M2 work is balance-bible content-design (adding depth), NOT a Claude Code implementation prompt against existing mechanics. No CF-89 sub-surface designed here.

### 4 — 5-run note read (CF-85 PR-B worked)
The 5-run confusion note (verbatim across all 5 runs) read against the shipped PR-B surfaces: **clause 2 MOVED** — S2b (post-combat opponent-build reveal) landed, so the "who/what did I fight" confusion is addressed. **Clauses 1 + 3 INTACT** and collapse to a SINGLE question — "why is this arrangement good" — which is exactly CF-89's choosing/why affordance. PR-B (the four legibility surfaces) worked as scoped; the residual is CF-89, not a PR-B defect.

### 5 — Master-dev clause-1 → attribution scoping prediction: DISPOSITIONED, NO ordinal (drifts +0)
An in-chat master-dev prediction mapped confusion clause 1 to Surface 1 (item attribution). The 5-run note refuted it: attribution answers "which item dealt this" (mechanical / legibility), whereas clause 1 is strategic ("why is this good") → clause 1 collapses to CF-89 (§ 4), not S1.

**Disposition — NO ordinal (mints no Drift). Ruled AGAINST the +1 lean, on ONE forcing criterion: item 5 is a design PREDICTION, not an assertion-time-falsifiable state-fact.** The counted master-dev claim-accuracy drifts of this arc — Catches 82/83/84 + Catch 81 / Drifts 62/63/64 + 61 (decision-log.md 2026-07-21 § "CF-85 ARC CLAIM-ACCURACY RATIFIED …") — were FALSE ASSERTIONS about grep/code-verifiable state (paths, prop-threading, ordinal-in-canon, removed-file liveness), falsifiable AT ASSERTION TIME. Item 5's claim is a DESIGN PREDICTION about a surface's comprehension EFFECT, whose truth-value exists ONLY post-playtest; a prediction disproven by its own test is the design loop working — the Hypothesis-in-Context posture (decision-log.md 2026-07-21 § "CF-85 ARC CLAIM-ACCURACY RATIFIED …", HELD: a suspected effect belongs as the hypothesis a test evaluates, and its refutation is the expected outcome, not a claim-accuracy defect). Minting a Drift here would flood the counter with design-iteration noise. Master-dev may overturn per the standing "disposition is the master-dev chat's call" note.

**Correction to this entry's first draft (pre-commit; this text supersedes it).** The initial draft rested +0 partly on two legs that are NOT load-bearing and are STRUCK: (i) "caught pre-propagation" and (ii) the prediction had "no propagation teeth." This arc's Catches 82/83/84 were themselves caught in-gate — pre-propagation, before any code — AND had zero shipped defects (no teeth), yet they COUNTED; so neither "caught early" nor "no teeth" can be what confers +0. Only the categorical design-prediction-vs-state-fact distinction can. The pre-propagation self-catch exclusion IS a real canon convention (decision-log.md 2026-07-07 § "M1 dashboard exit-gate CLOSED …"; 2026-07-11 § "Icon batch 2 (Uncommon) landed …"; 2026-07-12 § "CF-67 OPENED (corrected) …"), but it governs a SELF-catch of one actor's own in-flight drafting error before it enters the log — NOT a cross-party prediction refuted by playtest. Applying it to item 5 was an over-application (the same error canon corrected as a supersession at decision-log.md 2026-07-11 § "Drift 32 logged …"), and the first draft mis-anchored its citation to § "CF-85 ARC CLAIM-ACCURACY RATIFIED", which does not contain the convention. Verdict UNCHANGED: +0, on the categorical leg alone.

### 6 — Master-dev enumeration watch (NO ordinal)
The Q3 verdict trichotomy ((a) no system / (b) exists + named / (c) latent dead field) was correctly scoped to the STAT axis. The trigger-economy counter (§ 2) surfaced as a caveat OUTSIDE the three buckets rather than being force-fit into one — the correct handling of an enumeration whose axis does not span the finding. Recorded as a watch on enumeration-axis scoping; mints nothing.

### Counter
Light entry (arc disposition; no PR, no merge). Ordinal walk (live from canon; tip `61d04c7` = decision-log.md 2026-07-21 § "`replay_viewed` telemetry CF OPENED at CF-90 …" carrying **85/34/10/65/53**): highest **Catch 85**, **Rule 34**, **Pattern 10**, **Drift 65**, **CF-90** (grep-confirmed). Deltas by ID: catches **+0**; rules **+0**; patterns **+0**; drifts **+0** (item 5 dispositioned no-ordinal — design-prediction validated by playtest, not a verifiable-state claim; item 6 a watch); open-CFs **+0** (NO CF opened — the trigger-economy caveat opens none; NONE closed — CF-89 stays OPEN, CF-85 stays OPEN on surface 2c). Running line: **85/34/10/65/53 → 85/34/10/65/53** — catches **85** / rules **34** / patterns **10** / drifts **65** / open-CFs **53** (UNCHANGED). Docs-only disposition; the counter anchor is this entry's own docs commit.

Open-CF touch (delta only; no state change):
- **CF-89** — "why is this arrangement good" choosing/why affordance. Stays OPEN, deferred to M2; deferral now evidence-backed (§ 1). M2 work is balance-bible content-design, not a Claude Code prompt.
...
### Findings (Phase-1 report; each load-bearing fact verbatim-evidenced there)
- **Surface-2 data FULLY AVAILABLE client-side, both legs.** `makeGhostForRound` (`apps/client/src/combat/ghost.ts:66-129`) returns a complete `GhostTemplate` — real class (`round % 2`), real item ids + anchor + rotation — a pure deterministic fn of `(baseSeed, round, bagDimensions)`, recomputable at any phase. The item-11 deferral was costed for a pre-combat live inspector gdd.md §14 forbids anyway; "real data once M2 storage lands" is PROVENANCE (procedural placeholder vs player-submitted), NOT availability.
- **Current opponent surfacing is HARDCODED FAKE.** Pre-combat intent panel (`LeftRail.tsx:175-207`): literal "Ghost", the placeholder sub-line, static `iron-sword` + `wooden-shield` regardless of the real ghost (mobile identical, `MobileTopBar.tsx:12-17`). NO post-combat opponent-build view exists (`RoundResolution` + `RunEndScreen` grep clean). Even §14's sanctioned reveal (apparent class + 1–2 silhouettes) is not yet real.
- **Surface-1 data path CONFIRMED (item 11's S1 stands).** Combat is client-simulated (`runCombat` in the `CombatOverlay` memo); the full `CombatResult.events` stream is client-local, and every `damage` / `heal` / `item_trigger` carries per-tick `source`. Bounds: `source` is a `placementId` (`ItemRef`), not an itemId → an item attribution needs a placement→itemId lookup against the side's bag (player snapshot already retained; ghost placements retainable from `buildCombatInput`); `status_tick` is source-less → DoT attributes to an item only by correlating the earlier `status_apply`.
- **CF-60 CLOSED** (decision-log.md 2026-07-10 § "CF 60 CLOSED …", PR 33, merge `dee67ee`) — arranging teal adjacency-glow + combat burst shipped/live (`AdjacencyGlow.tsx`, `BagBoard.tsx:72` / `:150`). Generic `on_adjacent_trigger` participation is visible pre-combat. `PLAN-cf60-adjacency-signal.md` removed from the repo (2026-07-08 practice).
- **CF-50 OPEN**, schema-only, no emit site, no tutorial subsystem (decision-log.md 2026-05-23 § "M1.5c PR 1 CLOSED …").

### Redraw
1. **PR-B (client-only, no corpus, no wire) = FOUR surfaces.** Surface 1 (item attribution on trigger, carrying the two attribution bounds above); Surface 2a (real intent panel — real class + real 1–2 marquee silhouettes, §14-legal, de-fakes the panel, consumes the `GhostTemplate` / `Combatant` SHAPE, not the generator); Surface 2b (NEW post-combat "view opponent build" on `RoundResolution` / `RunEndScreen` — the PRIMARY clause-2 leg; §14's pre-combat restriction is N/A post-combat, gdd.md §12's "view opponent build" use is un-tagged, consumes the `GhostTemplate` shape); Surface 3 (replace the hardcoded "Round 4 · ±1 trophy" at `LeftRail.tsx:196` with real round + real win/loss `trophyDeltaFor` deltas + contract objective `state.contractName` / `state.contractText`).
2. **Surface 2c (player-submitted ghost provenance / persistent storage) stays M2** — genuine lifetime / provenance coupling, the real content of the original deferral. CF-85 STAYS OPEN on 2c after PR-B merges.
3. **Strategy clause SPLIT.** "Strategic logic behind choosing and placing" divides: the PLACING half (live placed-adjacency + named recipes) is COVERED by CF-60 (shipped) + `RecipeGlow`; the CHOOSING + explanatory-WHY half (why-buy-this, what-would-synergize-if-placed, reachable-recipes-from-stock, class-affinity leverage, trigger-cadence) is UNCOVERED → CF-89 OPENED, design-scope (needs a Phase-1 design pass to define "why is this arrangement good" as acceptance; the definition was deliberately held out of the read-only pass). CF-89 does NOT ride PR-B; its priority vs the persistence track is a later call. Candidate M2.

### PR split
PR-B is NOT split further — 2b is the primary clause-2 leg; splitting it out reproduces the deferral mistake (surfaces that miss the load-bearing clause). All four surfaces are client-only, no corpus regen, no wire (combat client-simulated). PR-A-before-PR-B is satisfied (PR-A merged `32558e7`).

### Catch 81 + Drift 61 + Rule 32 scope
The CF-85 Phase-1 investigation prompt listed `PLAN-cf60-adjacency-signal.md` as a live consumable input; the file was removed per the 2026-07-08 practice and CF-60 has been CLOSED since 2026-07-10. Master-dev claim-accuracy, caught IN-GATE by handoff-verify + the Phase-1 Q5 verify-instruction before canon — a caught-early hand-off claim still counts (Catch 64 / Catch 66 halt-gate precedent, decision-log.md 2026-07-19 § "CF-83 FIX DIRECTION CORRECTED …" item 189-leg; distinct from the CF-67 code-defect axis where caught-in-gate = +0). Class A / Rule 30 (an unverified factual claim about shipped state in a hand-off artifact). Drift 61 pairs it (Topic 2, one drift per master-dev catch, Catch 66 / Drift 46 precedent). Root of Drift 61: the project knowledge base RETAINS artifacts removed from the live repo, so a `project_knowledge_search` hit is CARRIED STATE of Rule 32's own class and needs a live-repo existence check before citation as an input. NO NEW RULE ORDINAL — recorded as a Rule 32 SCOPE note (Rule 32 covers `project_knowledge_search` hits, not only chat-carried state); a dedicated convention codifies on a second instance. The CF-60-open framing in the same prompt shares this root but was self-corrected by Q5's verify-instruction (no separate catch — the prompt caught its own premise, CF-67 logic).

### Counter
Ordinal walk (live from canon at tip `16dee1a`, greps run this session): highest **Catch 80**, **Rule 33**, **Drift 60**, **Pattern 10**, **CF-88**; an explicit grep for any `CF-89`-or-higher token → **0** hits (no phantom higher ordinals). Baseline: main tip `16dee1a`; governing counter entry decision-log.md 2026-07-20 § "CF-83 RAMP + CF-84 DRAW SEMANTICS CLOSED …" carrying **80/33/10/60/51**. Deltas by ID: catches **+1** (Catch 81); rules **+0** (Rule 32 scope note — no ordinal); patterns **+0**; drifts **+1** (Drift 61); open-CFs **+1** (CF-89 OPENED; NONE closed — a scope redraw is NOT a closure, CF-85 stays OPEN on surface 2c). Running line: **80/33/10/60/51 → 81/33/10/61/52** — catches **81** / rules **33** / patterns **10** / drifts **61** / open-CFs **52**. Docs-only supersession — no PR, no merge, no code; the counter anchor is this entry's own docs commit.

Open-CF touch (delta only; the full backlog is carried unchanged per decision-log.md 2026-07-20 § "CF-83 RAMP + CF-84 DRAW SEMANTICS CLOSED …"):
- **CF-85** — client legibility. SCOPE REDRAWN (this entry): surfaces 1 + 2a + 2b + 3 → PR-B; surface 2c → M2. STAYS OPEN on 2c.
- **CF-89** — "why is this arrangement good": the choosing + explanatory-WHY legibility half. OPENED (this entry), design-scope, candidate M2. Does NOT ride PR-B.

---

## 2026-07-20 — CF-83 RAMP + CF-84 DRAW SEMANTICS CLOSED (PR-A merged `32558e7`): Clause 1a MET (390→0, determinism SHA-equal), Clause 1b = NAMED post-deploy verification (baseline 12.7% / target <1%); 3 Codex-review findings ONE-ROOT (damage-def / ramp-ordering / KO-flash), all in-gate → catches +0 (CF-67 principle); pre-existing damage-definition divergence attributed; corpus-proves-construction-not-consequence reinforces POPULATION CAVEAT (ii); CF-88 OPENED (endReason cause-vs-tick, 16-combat gap); Rule 33 correlated-site sweep scoped by this closure

PR-A merged to `main` at `32558e7` (`--no-ff`; parents `cfc7cfa` + `91baf7a`; GitHub PR \#54 `merged: true`, merged_at 2026-07-20). CLOSES CF-83 (ceiling-decrement resolution ramp) + CF-84 (honest draw semantics), ratified at decision-log.md 2026-07-19 § "CF-83 RAMP + CF-84 DRAW SEMANTICS RATIFIED …" (`cfc7cfa`). Seven commits: `8744da2` (ramp + CF-84 render + `endReason` threading), `1e0dce4` (Clause-1a coverage repurpose), `9c96f30` (Stage-2 regen), then the three review fixes — `3500df6`, `1851fe9` + `dbad72e`, `91baf7a`. Codex review: 4 rounds, ceiling never tripped. Source basis: the four rounds' verified outputs (below), the Stage-2/Stage-3 corpus scans, and the merge verification. Citable by date + section as an independent referent; `32558e7` is the artifact anchor.

### 1 — CF-83 + CF-84 CLOSED; Clause 1a MET, Clause 1b = NAMED post-deploy verification
Clause 1a (construction guarantee; ratified instrument = the regenerated corpus): `endedAtTick == 600` count **390 → 0** across all 1301 combat_end events — the regen coverage gate reports `tick-cap draw (===0 / 1a): 0 [OK]`, and an independent per-combat scan confirms 0. Determinism re-proved by a byte-identical double-write (SHA-256 `733f1ecd…`, identical across two consecutive `generate-fixtures` runs). Final corpus distribution 643 player_win / 449 ghost_win / 209 draw (1301 total, sum verified). CF-84 render shipped: RoundResolution is 3-way (a draw renders **DRAW**, not the pre-CF-84 "LOST"/"DEFEAT"); economy UNCHANGED — `applyCombatOutcome` untouched, verified absent from every PR-A diff. Both CLOSE on Clause 1a + the shipped render. **Clause 1b** (player-facing resolution — real play, PostHog, tick-cap draws over all combats) is a NAMED POST-DEPLOY VERIFICATION: baseline **12.7%** (33/260), target **< 1%**, un-measurable until the fix deploys and real play accrues. NOT held open — precedent decision-log.md 2026-07-13 § "M1 EXIT GATE CLOSED" (the run-length criterion amended to defer the real-play measure rather than block closure on data that does not yet exist).

### 2 — THREE REVIEW FINDINGS, ONE ROOT (CF-84 made the invisible population visible)
CF-84's honest render + the ramp's `ramp_tick` (added to MEANINGFUL_EVENT_TYPES) routed the previously zero-content-fast-skipped **mutual-KO-draw population** through the full client + telemetry + playback path for the first time — surfacing three LATENT defects on exactly that population, each caught in-gate and fixed red-then-green:
- **Round 1 (P2, `3500df6`):** the client DEALT/TAKEN computed `initialHp − finalHp` (net HP lost), absorbing the ramp drain and disagreeing with `round_end` telemetry's event-based `computeDamageStats`. Fix A exported `computeDamageStats` from `@packbreaker/sim`; the client now uses one gross-item-damage definition for display + telemetry (a ramp draw reads 0/0). Tests: ramp draw → 0/0 (RED under the old delta, `expected 30 to be +0`); damage+heal → gross 12 not net 4 (`expected 4 to be 12`).
- **Round 2 (P2, `1851fe9` + Stage-3 `dbad72e`):** `applyResolutionRamp` ran between `runTick` and the death check unconditionally, so an item KO at tick ≥ 500 was drained by the ramp too — flipping a decisive result into a FALSE DRAW. Guard: apply the ramp only when both sides survive `runTick` (`player.hp > 0 && ghost.hp > 0`). Tests: ghost item-KO@500 → player_win (RED, `expected 'draw' to be 'player_win'`); mirror → ghost_win; heal-revive → ramp_ko (regression lock, RED under an over-firing guard, `expected 501 to be 500`).
- **Round 3 (P3, `91baf7a`):** `CombatScene`'s KO flash read `player_win → ghost, else → player`, so a mutual-KO draw flashed only the player though both died. Fix: three-way `koFlashTargets` extracted to a pure module (testable without Phaser, the `anchorResolution.ts` pattern). Tests: draw flashes BOTH (RED under the old two-way branch, `expected 1 to be 2`); a decisive win flashes exactly one.
Round 4: CLEAN ("Didn't find any major issues").

### 3 — TWO AXES, BOTH WORKING: catches +0 (CF-67 principle)
This session recorded **five master-dev claim-drift catches** (76–80, decision-log.md 2026-07-19 § "CF-83 FIX DIRECTION CORRECTED …" — all claim-accuracy, zero shipped defects) and, in the SAME session, **three shipped-code defects** here — with **zero catches**. The distinction is the CF-67 principle (a review WORKING is not an escape; decision-log.md 2026-07-12 § "CF-67 Phase 2 CLOSED …", applied at decision-log.md 2026-07-18 § "CF-77 CLOSED — Phase 2 PR2 …" where 2 P2s took NO catch): a catch is a defect that ESCAPES to canon or production; all three review findings were caught in-gate and fixed before the merge SHA existed. The round-2 corpus over-count (a 6-combat heuristic corrected to 1 by a precise re-scan) was SELF-CORRECTED before canon → Rule-29-excluded (decision-log.md 2026-07-18 § "CF-68 next-target selection … Rule 29 …"). The ledger caught the drift; Codex caught the code; both axes did their jobs. **Catches +0.**

### 4 — PRE-EXISTING DAMAGE-DEFINITION DIVERGENCE (attributed, ramp-aggravated)
The round-1 finding is not purely a PR-A regression. The client's `finalHp`-delta (net HP lost) and the sim's `computeDamageStats` (gross event damage) were TWO different definitions that ALREADY disagreed for heal-involving combats before PR-A; the ramp made the disagreement SYSTEMATIC on the ramp population (~34.7% of combats). Fix A collapses them to one definition, correcting the pre-existing healing divergence as well. Attributed as pre-existing so the ledger does not read this as a defect PR-A introduced.

### 5 — CORPUS PROVES CONSTRUCTION, NOT CONSEQUENCE (finding; reinforces POPULATION CAVEAT ii)
The round-2 guard's corpus reach was **1** true outcome flip (`162-recipe-chaser-1162`, draw → ghost_win — the HARMLESS direction; draw and ghost_win cost the same heart). The DANGEROUS direction (a ghost item-KO'd at ≥500 while the player is alive → the ramp robs an EARNED player_win into a draw, costing a heart) has **ZERO** corpus instances — not because it cannot happen, but because the scripted strategies did not sample it. This reinforces POPULATION CAVEAT (ii) (decision-log.md 2026-07-19 § "CF-64 CLOSED …") and item 14 of the ratification entry: the corpus BOUNDS construction and does not PREDICT experience. Consequence stated plainly: the round-2 guard's tests pin the one population neither the frozen corpus nor Codex's diff-read touches. FINDING, not a rule — one PR, one root; the second-instance-across-PRs codification bar is not met.

### 6 — CF-88 OPENED: endReason cause-vs-tick (quantified, separated from this PR)
The round-2 guard exposed that `endReason` (tick-based, ratification item 7: a death at tick ≥ 500 is `ramp_ko` regardless of cause) mislabels item/status KOs at tick ≥ 500. Quantified on the Stage-3 corpus: `ramp_ko`-by-tick **451** vs `ramp_tick`-bearing combats **435** — a **16-combat gap** = item-KOs the guard correctly leaves un-ramped, yet item 7 labels them `ramp_ko` by tick. Ordinal walk SHOWN, not asserted (live grep of decision-log.md, not a carried number): `grep -oE 'CF-[0-9]+' decision-log.md | sort -u` → highest token **CF-87** (tail: … CF-85 / CF-86 / CF-87); `grep -cE 'CF-(8[89]|9[0-9]|[1-9][0-9]{2,})' decision-log.md` → **0** (no CF-88-or-higher anywhere in canon). Next free ordinal → **CF-88** OPENED — refine `endReason` to cause-based labeling (telemetry-semantics scope, M2/M3). DELIBERATELY SEPARATED from this closing entry: editing item 7 inside a merge-closing entry would re-ratify a ratified decision as a rider (the rider pattern this discipline guards against). OPEN.

### 7 — RULE 33 CORRELATED-SITE SWEEP (the "middle, not top" watch item, now scoped by this closure)
Rule 33 (decision-log.md 2026-07-19 § "CF-83 RAMP + CF-84 DRAW SEMANTICS RATIFIED …" item 15) scopes a correlated-site update by the SUPERSEDING ENTRY, not by fact or position. This closure supersedes every cache/topic-file claim that CF-83/CF-84 are OPEN or that the ramp is ratified-but-unshipped — the "middle, not top" watch item flagged at Rule 33's codification (the project_cf83_phase1 topic file's design framings; the project_m2_position OPEN-CF state). The sweep is scoped by THIS entry; the memory resync is the correlated-site follow-up, executed against this closing entry after the docs commit. No committed doc is edited by this entry.

### 8 — P3 KO-FLASH: TEST-LOCKED, not visual-confirmed
Unlike the M1.3.4b VFX precedents (visual-confirmed; Phaser is not unit-testable at the scene layer), the round-3 fix extracted the flash-target decision to a pure module `koFlashTargets(outcome)` — testable without Phaser. 4 tests: `player_win → {ghost}`, `ghost_win → {player}`, `draw → {player, ghost}` (RED under the old two-way branch), and decisive-win-flashes-exactly-one. The disposition is TEST-LOCKED, not visual-confirmed — recorded to distinguish it from the VFX precedents.

### Counter
Ordinal walk (live from canon; the counter BASELINE is the decision-log tip `cfc7cfa`, the 2026-07-19 ratification entry that carried `80/33/10/60/52`): highest Rule 33, Drift 60, Pattern 10 — no phantom higher ordinals; the CF walk is shown in item 6 (highest token CF-87, zero CF-88+ in canon, next free → CF-88). The Rule 13 + `tsc` gates ran on the BRANCH tip `91baf7a` before merge (a distinct SHA from the decision-log tip): Rule 13 triple-green (`25 successful, 25 total / 0 cached` ×3 cold-cache), `tsc -b --noEmit --force` exit 0.

| axis | baseline (`cfc7cfa`) | delta | total (this closing entry's docs commit) | IDs |
|---|---|---|---|---|
| catches | 80 | +0 | 80 | three review findings caught in-gate, none escaped (item 3); round-2 over-count self-corrected pre-canon (Rule 29) |
| rules | 33 | +0 | 33 | red-then-green is Rule 28 applied; pure-module extraction is existing practice |
| patterns | 10 | +0 | 10 | — |
| drifts | 60 | +0 | 60 | review surfaced code defects, not claim/process drift |
| open-CFs | 52 | −1 | 51 | CF-83 + CF-84 CLOSED (−2, item 1); CF-88 OPENED (+1, item 6) |

Running line: **80/33/10/60/52 → 80/33/10/60/51** — catches **80** / rules **33** / patterns **10** / drifts **60** / open-CFs **51**.

Anchor independence (citation-discipline amendment): the counter TOTAL (51) is reached when THIS closing entry commits — a docs SHA later than, and distinct from, the merge commit `32558e7`. `32558e7` is the ARTIFACT anchor (the merge that closed CF-83/CF-84; it touches no decision-log.md), NOT the counter anchor. The counter anchor is this entry's own docs commit, recorded here once it exists.

Open-CF re-enumeration (the CF-83 arc, one bullet per CF; the remaining backlog is unchanged and carried per decision-log.md 2026-07-18 § "CF-68 next-target selection …"):
- **CF-83** — resolution ramp. **CLOSED** at `32558e7` (this entry).
- **CF-84** — honest draw semantics. **CLOSED** at `32558e7` (this entry).
- **CF-85** — client legibility surfaces 1+3 JOIN / surface 2 (opponent-build) DEFERS. OPEN → PR-B (sequenced after this).
- **CF-86** — class-asymmetric round-1 resolution (Marauder `bonusBaseDamage`). OPEN, standalone.
- **CF-87** — boss-encounter wiring into real play (§15 carve-out). OPEN, M2.
- **CF-88** — `endReason` cause-vs-tick labeling (16-combat gap). OPEN (new, item 6), M2/M3.

---

## 2026-07-19 — CF-83 RAMP + CF-84 DRAW SEMANTICS RATIFIED (Phase 2 gate): ceiling-decrement ramp R=3 from t500 (trigger-bypass, event-emitting) SUPERSEDES 2125b82's "post-tick-~300 damage ramp"; CF-84 honest-render + NO economy change (4th-outcome-member REJECTED, additive `endReason`); success criterion RESTRUCTURED to Clause 1a/1b; Clause 2 RETARGETED to heart-costing-rate — REQUIRES NO CHANGE (CF-83 is a LEGIBILITY fix, not a difficulty fix); §15 boss carve-out SUSPENDED + CF-87 OPENED (boss wiring); CF-85 surfaces 1+3 JOIN / surface 2 DEFERS; PR-A/PR-B split; Rule 33 NEW + Drift 60

Docs-only ratification, insertion-only — the Phase 2 Rule 20 gate. Baseline: main tip `bad7b1e`; governing entry decision-log.md 2026-07-19 § "CF-83 FIX DIRECTION CORRECTED …". This SUPERSEDES named legs of decision-log.md 2026-07-19 § "COMBAT NON-RESOLUTION + LEGIBILITY diagnosed …" (landed `2125b82`): its damage-ramp mechanism wording (item 1), its "post-tick-~300" start (item 2), its single Clause 1 (item 8), and its Clause 2 expected-loss formula (item 9). No prior entry is edited. Source basis: the joint CF-84+CF-83 Phase 1 report (this session, tree clean at `bad7b1e` — T1–T5 / D1–D4 / M1 / S1), the PostHog table below, and the Step-0 + item-5/6/7 change-site verifications. No code, schema, migration, or fixture. Rule 20: no Phase 2 implementation prompt may cite any of this until it carries a real SHA.

RAW DATA (PostHog `combat_end`, 2026-07-19, n=260; reconciles to canon 218/35/7): window(endedAtTick)×outcome — <300 draw 1 / gw 2 / pw 197; 300–399 draw 1 / gw 3 / pw 14; 400–499 gw 2 / pw 6; 500–599 pw 1; ==600 draw 33. Full-HP ties among draws (damageDealt=0 AND damageTaken=0) 5/35=14.3%; corpus equivalent 224/390=57.4%. Derived (arithmetic verified this session): tick-cap draws 33/260=12.7%; sub-cap (mutual-KO) draws 2/260=0.77%; heart-costing (draw+ghost_win) 42/260=16.2%; alive at t300=60 (rescues 33, preempts 27), t400=42 (33, 9), t500=34 (33, 1).

### Step 0 — HP_max derived (halt gate cleared, R ratified unconditional)
Legal bag 6×4 = 24 cells. `passiveStats.maxHpBonus` inventory (verbatim, packages/content/src/items.ts): buckler 5 (1×1), iron-shield 8 (1×1), chainmail 12 (1×2), world-forged-heart 15 (1×1, legendary), tower-shield 18 (2×2), bloodmoon-plate 25 (2×2). Densest per-cell: world-forged-heart 15/cell (single, boss-reward), iron-shield 8/cell (unlimited, 1×1). Max Σ maxHpBonus over 24 cells = 1×WFH(15) + 23×iron-shield(8) = 15+184 = 199 → **HP_max = 30 + 199 = 229** (222 without the single legendary). Max sustained per-tick heal = 24 × healing-herb (`on_cooldown(80): heal 3`, 1×1) = 24×3/80 = **0.9/tick** (recorded for completeness; the ratified ramp's termination proof has no heal term). **HP_max = 229 ≤ 300 → R = 3 RATIFIED**, per R ≥ HP_max/100 = 2.29. NO HALT.

### 1 — RAMP MECHANISM RATIFIED: ceiling-decrement, symmetric (SUPERSEDES 2125b82's "damage ramp")
From the ramp-start tick, decrement BOTH combatants' current HP AND their heal ceiling (effective max HP) by R per tick, as a direct state mutation. SUPERSEDES the "post-tick damage ramp" wording of decision-log.md 2026-07-19 § "COMBAT NON-RESOLUTION …" (`2125b82`): "damage ramp" implies routing through the damage path, which item 5 shows is broken. Selected over the two symmetric damage-ramp candidates because it is the ONLY design (Phase 1 D1 "Design 3") whose termination proof contains no heal term — heals cap against `startingHp` in `packages/sim/src/combat.ts` (`applyDamage` lifesteal cap + `resolveEffect` heal cap), so a falling ceiling makes healing structurally unable to outpace it, and termination is independent of item composition (the property that survives future content).

### 2 — START TICK 500 (SUPERSEDES 2125b82's "post-tick-~300")
The ramp only affects combats still alive when it fires. Per the window×outcome table: a t300 start rescues 33 stalls but PREEMPTS 27 combats that resolve on their own; a t500 start rescues the same 33 and preempts 1. All 7 ghost_wins and both sub-cap draws end before t500, so a t500 ramp touches NEITHER population. Preemption ≠ a flipped result: a symmetric ramp kills the lower-HP side first, preserving the leader — the single exposed combat (the pw at 500–599) flips only if the player was behind at t500 and would have come back. Worst-case exposure 1/260 = 0.38%.

### 3 — RATE R = 3 (unconditional, per Step 0)
R = 3, from R ≥ HP_max/100 with HP_max = 229 (Step 0, recorded so a future reader re-derives it). At R=3 from t500 the heal ceiling zeroes at 500 + ⌈229/3⌉ = tick 577 ≤ 600 for the absolute-max bag; the modal 30-HP stall zeroes at 500 + ⌈30/3⌉ = tick 510. Legibility floor: at R=3 a 30-HP stall resolves in ten ticks = one simulated second of visible ramp (`TICKS_PER_SECOND` 10) — the argument against a later start compensated by a steeper rate.

### 4 — ASYMMETRIC ELIMINATED, SYMMETRIC CONFIRMED
The Phase 1's leading candidate (asymmetric, player −4 / ghost −3, forcing a decisive winner) was selected against a CORPUS full-HP-tie rate of 57.4%. Real play is 14.3% — 4× lower. 30 of 35 real draws carry net HP asymmetry, so a symmetric ramp resolves them toward whoever is actually ahead; the asymmetric ramp would OVERRIDE real signal with a forced player loss to manufacture ghost_wins for a band item 9 shows was already satisfied. The Phase 1's M1 predictions (the asymmetric candidate: 305/390 ghost_win on the corpus) are CORPUS-DERIVED and NON-TRANSFERRING — the corpus's 57.4% tie rate is the exact reason a class-neutral symmetric ramp read as ineffective there but is correct for real play.

### 5 — TRIGGER-BYPASS REQUIRED (load-bearing; VERIFIED at the change site)
The ramp must be direct HP subtraction that does NOT enter trigger resolution. Routed through the damage path it fires `on_taken_damage` reactions. VERIFIED (packages/content/src/items.ts `healing-salve`): `{ type: 'on_taken_damage', effects: [{ type: 'heal', amount: 3, target: 'self' }] }` — heals exactly 3 on taken damage, net-zeroing a 3/tick damage-ramp into an infinite stall and destroying the termination proof. The ceiling-decrement (item 1) sidesteps this by mutating HP + ceiling directly, outside `applyDamage`.

### 6 — EVENT EMISSION REQUIRED (VERIFIED at the change site)
Bypassing trigger resolution must NOT mean bypassing the event log — a silent bilateral drain is worse legibility than the stall it replaces. The ramp emits a visible `CombatEvent`; its representation (new union member vs. reuse of an existing type) is a Phase 2 design question, recorded here with its schema implications only. Stage 2 regenerates the corpus for the balance change regardless, so a new event type's fixture cost is already paid. ⚠ VERIFIED (apps/client/src/combat/CombatOverlay.tsx): the zero-content fast-skip is `isZeroContent = hasNoMeaningfulEvents && result.outcome === 'draw'`, with `hasNoMeaningfulEvents = !result.events.some(e => MEANINGFUL_EVENT_TYPES.has(e.type))` and `MEANINGFUL_EVENT_TYPES = {damage, heal, status_apply, status_tick, item_trigger, stun_consumed, buff_apply, buff_remove}` (combat_start/combat_end excluded). CONSEQUENCE: ramp-resolved player_win/ghost_win are unaffected (outcome ≠ 'draw'), but a mutual-KO draw whose ONLY events are ramp events would satisfy `hasNoMeaningfulEvents` and skip Phaser — defeating the CF-84 legibility fix on exactly the population it exists to render honestly. REQUIRED: the ramp's event type MUST be added to `MEANINGFUL_EVENT_TYPES` (→ 9 members).

### 7 — CF-84 RATIFIED: honest render, NO economy change; additive `endReason`; Rule 31 order
Discriminator: `endedAtTick == 600` ⟺ timeout, `'draw'` with `endedAtTick < 600` ⟺ mutual KO (T1: `simulateCombat`'s loop bound `state.tick < MAX_COMBAT_TICKS` guarantees a mutual KO returns `endedAtTick ≤ 599`). Zero schema cost, zero fixture churn. A 4th `CombatOutcome` member is REJECTED — the timeout outcome becomes dead-but-required-for-replay once the ramp guarantees termination, and an exhaustiveness-churn cost for an unreachable member is unwarranted; real-play mutual-KO rate is 0.77% (2/260), so no distinct consequence is introduced (both sides died, both pay the existing loss). The deliverable is the render change: stop rendering a draw as "LOST". ADDITIVE `endReason` on the `combat_end` telemetry event is ALSO ratified (CF-41 additive-telemetry-field precedent) — once the ramp lands, `endedAtTick == 600` is unreachable, so telemetry needs `endReason` to distinguish a ramp-driven resolution from an item-driven one, which is how Clause 1b's post-fix reads are interpreted. ⚠ RULE 31 (VERIFIED): the server telemetry `combat_end` object is `.strict()` (apps/server/src/validation/telemetryBatch.ts — module comment "object is .strict() (no passthrough — unknown keys reject)"; the combat_end object carries `.strict()`). Transition-window: (a) client emits `endReason` BEFORE the server tolerates it → `.strict()` rejects the unknown key → the combat_end event fails validation (client-first BREAKS combat_end ingestion); (b) server adds `endReason` as OPTIONAL first → it accepts combat_end with OR without the field, both directions validate. RATIFIED ORDER: server-side `endReason` tolerance ships FIRST (server before client, or same PR merged server-first), matching Rule 31 / the CF-68 PR-A wire-contract precedent.

### 8 — SUCCESS CRITERION RESTRUCTURED (SUPERSEDES 2125b82's single Clause 1)
- **Clause 1a — construction guarantee.** Instrument: Stage-2 corpus. Target: ZERO `endedAtTick == 600` against the Stage-1 baseline of 390.
- **Clause 1b — player-facing resolution.** Instrument: real play, PostHog. Metric: tick-cap draws (`endedAtTick == 600`) over all combats — NOT balance-bible.md § 2's all-draws definition (once CF-84 lands a mutual KO is a designed outcome; counting it against a combat-didn't-resolve guardrail penalizes the fix for working). Threshold: 1%, matching telemetry-plan.md §§ 3/5/6, § 2's balance-bug line, and the D1 dashboard alert; § 4's 0.5% is the outlier. Baseline to beat: 12.7% (33/260).
The 1a/1b split now has EMPIRICAL backing, not argument: item 14's two corpus-vs-real-play divergences demonstrate the corpus BOUNDS construction and does not PREDICT experience.

### 9 — CLAUSE 2 RETARGETED (SUPERSEDES 2125b82's expected-loss formula)
2125b82's formula (11 × ghost_win rate → 11×0.027 ≈ 0.3 → "the player cannot lose") used the wrong denominator. T2 proved `packages/sim/src/run/state.ts` `applyCombatOutcome` collapses BOTH `'draw'` and `'ghost_win'` to `roundOutcome = 'loss'` — identical hearts (−1), trophy (`trophyDeltaFor('loss')`), and gold (0). The heart-costing rate is draw + ghost_win = 16.2%, giving 11 × 0.162 = 1.78 expected heart losses against gdd.md § 1's 3 hearts — INSIDE Clause 2's own 1.65–2.75 "tense and completable" band. Clause 2 is retargeted to the heart-costing-outcome rate and, by that measure, REQUIRES NO CHANGE. Corroborated by the 2026-07-19 self-cert (decision-log.md 2026-07-19 § "COMBAT NON-RESOLUTION …"): 44 round wins / 6 losses across 5 runs, zero eliminations, all completing. CONSEQUENCE (recorded plainly): CF-83 is a LEGIBILITY fix, not a difficulty fix — the stakes were always there; 12.7% of them arrived labeled "draw" and rendered "LOST" with no visible cause. Also: trophy loss is clamped to `−min(5, trophy)` (`packages/sim/src/run/ruleset.ts` `trophyDeltaFor`), so low-trophy early-round stalls cost less than the nominal 5 — do not model round-1 draws at a flat −5.

### 10 — §15 BOSS CARVE-OUT SUSPENDED; CF-87 OPENED
balance-bible.md § 15's ~30% boss-win target is unmeasurable while the encounter is never instantiated (governing entry item 6: real-play round 11 is a plain 40-HP marauder ghost). SUSPENDED, with the explicit note that Clause 2 covers ALL rounds until the boss exists. Highest existing CF at tip `bad7b1e` = CF-86 (grep-walked live); wiring the boss contract into real play is opened as **CF-87** against M2 — real scope, no bearing on combat resolution. OPEN.

### 11 — CF-85 SURFACES 1 + 3 JOIN THE ARC; SURFACE 2 DEFERS
S1 confirmed the data already reaches the client for all three. Item attribution on trigger (~1–2 d) and the runtime intent hint replacing the hardcoded "Round 4 · ±1 trophy" (~0.5–1 d) JOIN — both unblocked. Opponent-build-inspectable (~2–4 d) DEFERS: it carries the heaviest `makeGhostForRound` coupling and gets real data once M2 ghost storage lands. CF-85 stays OPEN on the deferred surface.

### 12 — CF-86 STAYS STANDALONE
Not folded into CF-83. The ceiling-decrement ramp is class-neutral by construction; the round-1 class gap is a shop-composition + legibility problem (governing entry item 5). Folding it in would smuggle a content question into a termination fix.

### 13 — PR SPLIT RATIFIED
**PR-A:** CF-83 ramp + CF-84 semantics — they share ONE Stage-2 regeneration; splitting forces two, the second's baseline confounded by the first's churn (the exact condition the three-stage disposition, decision-log.md 2026-07-19 § "CF-64 CLOSED …", exists to prevent). **PR-B:** CF-85 surfaces 1 + 3, client-only, no corpus impact. Merge order PR-A before PR-B. Per Rule 31, the only wire contract crossing the boundary is item 7's `endReason` (server-first within PR-A).

### 14 — CORPUS-VS-REAL-PLAY DIVERGENCE RECORDED (two quantities; NO catch, Rule 29)
Full-HP-tie rate among draws: corpus 57.4% vs real play 14.3%. FINDING 1's "≥50.4% of stalls exchange zero net damage on both sides" (197/391) is the SAME corpus quantity; its real-play counterpart is 14.3%. POPULATION CAVEAT (ii) (decision-log.md 2026-07-19 § "CF-64 CLOSED …") already frames corpus rates as an upper bound and did its job (Rule 29 — no catch), but FINDING 1's figure reads as a general claim about stalls and needs its real-play counterpart recorded alongside it.

### 15 — RULE 33 NEW (correlated-site update) + Drift 60
> **Rule 33 — correlated-site update.** When a document is updated against a superseding canon entry, the sweep covers every claim in that document the ENTRY supersedes, not only the claim that prompted the update. Scope by superseding entry, not by fact. Position within the document is incidental.

Codified. Bend-note basis: the shape is structurally generic (any correlated-site update), the discipline is low-burden (sweep by entry, not fact), and the surface recurs on every supersession. The codification rests on THIS session's demonstration — no committed entry establishes a prior instance count, so none is asserted: a fact-scoped correlated-site sweep of project_cf83_phase1.md missed a stale line asserting a DIFFERENT superseded item of the same entry (the byte-delta directional check, superseded by the governing entry's item 8), and the entry-scoped re-sweep then found FIVE further sites the fact-scoped pass structurally could not reach. The earlier fact-scoped wording is SUPERSEDED, not amended. PAIRED DRIFT 60 (Topic 2, master-dev): master-dev named this discipline as governing a cache-file correction and, in the same message, authored a fact-scoped prompt that violated it. Substantive under Rule 15 — the surviving line would have told a future reader to read success as failure (the byte-delta directional check, superseded by the governing entry's item 8). Caught at the gate by Claude Code; per Rule 15 the gate firing does not null it. Master-dev ruled it counts.

### 16 — PATTERN 10 REMEDY APPLIED PROSPECTIVELY (referenced, no ordinal)
Pattern 10's ratified remedy (state the generative mechanism, then test the draft against a variant sharing the mechanism but not the surface) has been DIAGNOSTIC in prior instances — applied after a narrow rule let the next instance escape. Here it was applied to Rule 33's draft BEFORE codification and changed the wording that ships (scope-by-entry, not scope-by-fact). First PROSPECTIVE use; referenced instance, no new ordinal.

### 17 — NO-CATCH DISPOSITIONS (recorded to disambiguate)
- **(a)** A master-dev hypothesis that resolving a stall as ghost_win would change the trophy delta was refuted by T2 (both `'draw'` and `'ghost_win'` → `roundOutcome 'loss'`, identical −5 trophy). It sat under an explicit "HYPOTHESES TO EVALUATE, NOT PREMISES" heading and was worded conditionally — the disclaimer owned the job (Rule 29 Instance-2 exclusion). NO CATCH.
- **(b)** The Phase 1's T4 charged the 4th-union-member path with a 224-fixture re-baseline that Stage 2 performs anyway (a double-count). Dispositioned at master-dev review before reaching canon; the conclusion (Path B cheaper) was unaffected. NO CATCH, per the CF-67 ruling that a review working is not an escape.

### Counter
Ordinal walk (live from canon at tip `bad7b1e`): highest Rule 32, Drift 59, CF-86, Pattern 10 — no phantom higher ordinals. Baseline: main tip `bad7b1e`; governing entry decision-log.md 2026-07-19 § "CF-83 FIX DIRECTION CORRECTED …" — 80/32/10/59/51 → **80/33/10/60/52** — catches **80** / rules **33** / patterns **10** / drifts **60** / open-CFs **52**. Delta: catches **+0** (design ratification; the two no-catch dispositions in item 17); rules **+1** (Rule 33 — correlated-site update, item 15); patterns **+0** (Pattern 10 prospective instance referenced, item 16); drifts **+1** (Drift 60 — item 15's paired Topic-2 master-dev drift); open-CFs **+1** (CF-87 boss-wiring OPENED, item 10; NONE closed — CF-83/84/85/86 all stay OPEN, a design ratification is not a closure). Docs-only Phase 2 gate — no PR, no merge, no code.

---

## 2026-07-19 — CF-83 FIX DIRECTION CORRECTED (ghost-HP-curve leg DROPPED as un-reaching + delete-at-M2; shared-sim post-tick ramp RETAINED) + round-1 "cannot-resolve-by-construction" premise SUPERSEDED by real-play PostHog; class-asymmetry RE-ATTRIBUTED to Marauder `bonusBaseDamage` (not shop affinity); CF-86 OPENED; Catches 76–80 (all master-dev claim-accuracy, zero shipped defects), Drifts 55–59; Rule 28 amendment HELD; CF-84 real-play instance; read-only pass carried (G1/G2/C1–C4/R1–R3/N1/T1/V2/V3)

Docs-only supersession, insertion-only. Baseline: main tip `38007f7`; governing entries decision-log.md 2026-07-19 § "COMBAT NON-RESOLUTION + LEGIBILITY diagnosed …" (landed `2125b82`) and decision-log.md 2026-07-19 § "CF-64 CLOSED …" (tip `38007f7`). Neither entry is edited — both stay readable as accurate-at-authoring-time; the named legs below are SUPERSEDED, not rewritten. Source basis: this session's read-only pass (tip `38007f7`, zero mutations — G1/G2/C1/C2/C3/R1/R2/R3/C4/N1/T1/V2/V3) plus the raw data below. No code, schema, migration, or fixture touched. Rule 20 gate: no CF-83 implementation prompt may cite this until it carries a real SHA.

RAW DATA (cite as such, with date and n): PostHog, 2026-07-19, event `combat_end`, filtered `round = 1`, n = 28. Outcomes — player_win 14 (avg_tick 317.5, min 180, max 500); draw 12 (avg_tick 575, min 300, max 600); ghost_win 2 (avg_tick 310). Tick buckets, 50-tick — 150→2, 200→1, 250→1, 300→9, 400→1, 450→2, 500→1, 600→11. Class split (inner join `combat_end` round=1 → `run_start` on runId) — marauder 4/16 stalls (25.0%); tinker 7/10 (70.0%); 2 of 28 did not join.

### Item 1 — round-1 "cannot-resolve-by-construction" premise SUPERSEDED (Catch 76 + Drift 55)
The governing entry (landed `2125b82`) states "Measured per-item gross output is 0.27/s → 111s → round 1 CANNOT resolve within the cap by construction," and the CF-64 close entry's POPULATION CAVEAT line (iii) (tip `38007f7` — its true location) marks that leg "POPULATION-INDEPENDENT … load-bearing." REFUTED by the raw data: 16 of 28 real-play round-1 combats resolve within the cap (14 player_win + 2 ghost_win), fastest at tick 180; real-play round-1 stall rate 42.9% (12/28), not 100%. Defect: 0.27/s is a POOL MEAN over a bimodal distribution (item 2's tick shape — 17 combats t150–500, mode 9 at bucket 300, 11 at exactly 600, the 500–600 range EMPTY); a mean cannot support a universal "cannot, by construction" claim about a single draw whose pool members straddle the threshold. WITHDRAWN: the CF-64 close entry's POPULATION CAVEAT line (iii). Replacement — narrower and accurate: round-1 gold (4g; gdd.md § 2, balance-bible.md § 3) buys exactly one 3g common, and per the pass census 16 of the 20 commons cannot solo-resolve a 30-HP opponent within the cap, so the MODAL round-1 build stalls — a distribution statement, not a universal one. Class A / Rule 30 (re-amended, decision-log.md 2026-07-19 § "COMBAT NON-RESOLUTION …"): the falsifying segment (`combat_end` by round) was queryable at authoring and unqueried.

### Item 2 — FABRICATED PATH in a cause-refutation (Catch 77 + Drift 56; Pattern 10 lineage, referenced)
The governing entry's FINDING 1 § 5-cause-refutation leg (b) (landed `2125b82`) cites `packages/sim/src/combat/ghost.ts` `makeGhostForRound`. That path DOES NOT EXIST (`ls` → "No such file or directory"). The symbol is defined in `apps/client/src/combat/ghost.ts` `makeGhostForRound`; `git log --follow` shows birth at `9a4db67`, `--diff-filter=R` empty (never moved). Same shape as the fabricated-path catch at decision-log.md 2026-07-16 § "Catch 64 …" (a package prefix synthesized onto a bare filename) — and it sits inside the very `2125b82` entry whose PROCESS section RE-AMENDED Rule 30 to catch exactly this class of ground-truth-checkable claim (Rule 30 first codified one arc earlier, decision-log.md 2026-07-18). Class A / Rule 30. Further instance of the codification-to-surface-feature / under-generalization pattern (the `Catch 64 → Catch 66` lineage recorded at Rule 30's codification) — referenced, no new ordinal.

### Item 3 — CF-83 FIX DIRECTION CORRECTED: ghost-HP-curve leg DROPPED, shared-sim ramp RETAINED
The governing entry (landed `2125b82`) ratified a two-leg fix: (leg 1) invert the ghost-HP curve in `makeGhostForRound`; (leg 2) add a post-tick-~300 damage ramp. G2 of the pass: `makeGhostForRound`'s only production caller is `apps/client/src/combat/CombatOverlay.tsx` `buildCombatInput`; the 224-fixture corpus reaches `packages/sim/test/determinism/ghost-generator.ts` `generateProceduralGhost` exclusively. Leg 1 therefore moves REAL PLAY only and cannot move Clause 1's ratified instrument (the post-fix regenerated corpus) — and `makeGhostForRound` is M1.3.4a delete-at-M2 scaffolding (decision-log.md 2026-05-02 § M1.3.4a close), so inverting its curve invests in code slated for deletion. LEG 1 DROPPED. Leg 2 (post-tick ramp) lives in `packages/sim/src/combat.ts` `simulateCombat`, which both populations share; RETAINED as the sole lever. Two constraints recorded (ramp design is a separate Phase 1, out of scope here): (a) a symmetric ramp against two zero-damage bags yields a simultaneous mutual KO, not a resolution — CF-84 (draw semantics) must land first or concurrently; (b) the ramp is class-neutral and does not address item 5's Tinker gap.

### Item 4 — CLASS-ASYMMETRY MECHANISM RE-ATTRIBUTED (Catch 78 + Drift 57; VERIFIED, not pending)
A master-dev hand-off asserted the 45-point marauder/tinker round-1 stall gap was caused by class-affinity SHOP WEIGHTING (from balance-bible.md §§ 3/5/6) — conflating § 5's affinity TAGS with the shipped `item.classAffinity` field. REFUTED at the change site: in `packages/content/src/items.ts`, `classAffinity` is set on `whetstone` alone among the 20 commons; all four solo-resolvers are `classAffinity: null`; `packages/sim/src/run/shop.ts` `buildPool` shifts that one non-resolver by ≤45 of ~1200 pool points and never touches the resolver numerator. The actual mechanism is Marauder's `bonusBaseDamage: 1` COMBAT passive (`packages/content/src/classes.ts` CLASSES.marauder.passive; applied per damage effect in `packages/sim/src/combat.ts` `deriveSideStats` → `resolveEffect`, shared by both populations): it adds a fifth solo-resolver (iron-mace 22 → 33 damage over the cap) and accelerates every weapon (iron-sword 400→300, iron-dagger 450→300, wooden-club 360→300, hand-axe 400→320 tick), collapsing the four weapons onto tick 300 — which explains the observed bucket-300 mode. C2 cuts the OTHER way: `merchants-mark` (`packages/content/src/relics.ts`, `itemCostDelta: -1`) is Tinker-only and doubles Tinker's round-1 purchases (2 commons vs 1), so the purchasing lever favors Tinker against the observed gap. RECORDED AS VERIFIED — the pass settled it at the change site; not a pending hypothesis.

### Item 5 — STALL DECOMPOSITION: the CF-85 residual, now measurable (n=10 caveat)
C3 zero-solo-resolver probability in a 5-slot round-1 shop vs observed stall: Marauder 23.2% vs 25.0% (at the floor); Tinker 33.8% vs 70.0% — a ~36-point residual. Since a bought resolver does resolve (C1 census), the Tinker residual is players declining resolvers that WERE offered — a legibility quantity (CF-85), not a balance quantity. ⚠ n=10 on the Tinker cell: this is a DECOMPOSITION of a small sample, not a rate. Hypothesised cause, EXPLICITLY UNVERIFIED: Tinker's tool/gem/consumable affinity fantasy contains no standalone-damage common at round 1, so a Tinker buying "on theme" buys a non-resolver. Falsifiable prediction recorded for a future round-1 purchase-by-class query.

### Item 6 — the round-11 boss is DECORATIVE in real play; Clause 2's carve-out is UNMEASURABLE
R1/R2/R3 of the pass: `apps/client/src/run/useRun.ts` hardcodes `contractId: 'neutral'`; `packages/sim/src/combat.ts` `applyBossMutatorsToGhost` short-circuits on empty mutators; `packages/sim/src/run/state.ts` `startCombatFromGhostBuild` has no `apps/client` caller. Real-play round 11 is a plain 40-HP marauder ghost (`makeGhostForRound`), not balance-bible.md § 15's Forge Tyrant. The boss REWARD is round-keyed — `grantRelic`/`grantBossItem` gate on resolution-phase + round 11 + win, no contract/mutator check — so a neutral run collects it on any round-11 win. C4 corroborates from telemetry: `combat_end.damageDealt` on the real-play path is a full HP-delta (`initialGhostHp − finalHp.ghost`), so an observed round-11 player_win at damageDealt 40 implies a 40-HP opponent, not the 50-HP boss. CONSEQUENCE for CF-83: Clause 2's balance-bible.md § 15 ~30% boss-win carve-out is currently UNMEASURABLE in real play — flagged for disposition. The M1 exit gate is CLOSED (decision-log.md 2026-07-13 § "M1 EXIT GATE CLOSED") and stays closed; nothing here reopens it.

### Item 7 — TRAJECTORY-DIRECTION INFERENCE SUPERSEDED (Catch 79 + Drift 58; second instance of item 1's shape)
The CF-64 close entry's "Trajectory finding" section (tip `38007f7`) reads the +205663-byte net-larger regeneration delta as "regenerated combats emit MORE events, i.e. run LONGER," and treats it as independent corroboration of combat non-resolution. Superseded: the same section records the changed hunks span `buy_item` and `advance_phase` as well as the `perRoundCombatEvents` result line — the delta measures WHOLE-REPLAY divergence, not combat duration. The confound is directional: more stalls → more heart loss → earlier elimination → fewer rounds → SMALLER files; the metric cannot distinguish the two. Class A / Rule 30. Recorded second instance: the read-only pass's own N1 prose drew the same ambiguous conclusion from the same figure — evidence the wording actively misleads a competent reader — while N1's actual numbers cut the other way (timeouts 391→390 as total combats rose 1096→1105).

### Item 8 — STAGE-2 DIRECTIONAL CHECK REPLACED (stage number read from `38007f7`)
Per the CF-64 close entry's own header (tip `38007f7`), the disposition is THREE stages: Stage 0 (line-ending guard), Stage 1 (unchanged-code regen, done at that close), Stage 2 (post-balance-change regen). That entry's "Stage 2 directional expectation" section states a working CF-83 fix should produce a NET-NEGATIVE byte delta — backwards on its own logic: a working fix means players survive longer → LARGER files, so as written it instructs the next reader to read success as failure. REPLACEMENT for the Stage-2 check: count `endedAtTick == 600` combats in the post-fix regenerated corpus against the Stage-1 baseline; a working fix STRICTLY REDUCES that count. Rationale in Rule 28 terms: the byte delta returns the SAME sign whether the fix works or fails (direction-ambiguous per item 7), so it cannot produce a different result under the negation of what it claims; the tick-600 count does — it is falsifiable, the byte delta is not.

### Item 9 — N1 RECONCILED
Post-Stage-1 corpus (tip `38007f7`): 390 timeout-draws (`endedAtTick == 600`) / 4 sub-cap draws / 1105 total combats = 35.29% timeout rate. Canon's 391/5/1096 is the PRE-regeneration figure (governing entry, landed `2125b82`, and the CF-64 close POPULATION CAVEAT). Both are recorded with their corpus states (pre- vs post-Stage-1 regeneration); the delta is the Stage-1 regeneration effect, not a drift.

### Item 10 — CF-84 GAINS A REAL-PLAY INSTANCE
Round-1 draws are n=12 with 11 at exactly tick 600 and min_tick 300 — one sub-cap mutual KO (the in-tick death check, not a timeout). Canon previously recorded sub-cap mutual-KO draws only in the frozen corpus. This is the first observed in REAL play, and it is the CF-84 draw-semantics distinction (timeout vs mutual-KO) surfacing outside the corpus. No CF opened or closed; CF-84 stays OPEN.

### Item 11 — balance-bible.md § 3 INCOMPLETE (divergence recorded, no CF)
§ 3 states only "+50% pool weight" for class-affinity items. Shipped `packages/sim/src/run/shop.ts` `buildPool` implements the fuller § 14 three-tier model — own-affinity +50%, neutral base, other-class −25% — via `applyPct` = `Math.floor((base * (100 + pct)) / 100)`. § 14 governs and matches code; § 3 is an incomplete summary. Contributing cause of the item-4 mis-attribution. No CF opened.

### Item 12 — V2 THRESHOLD CONFLICT: five statements, two axes
balance-bible.md § 2 → 1%, metric = ALL draws / all combats. telemetry-plan.md § 4 → 0.5%, metric = tick-cap draws only. § 5 → 1%, tick-cap. § 6 → 1%, tick-cap. § 3 → 1%, tick-cap. Two conflicts: threshold (0.5% vs 1%) AND metric definition (all draws vs tick-cap draws). The governing entry's Clause 1 (landed `2125b82`) cites § 2 — the all-draws 1% figure — so the § 2 leg of its citation is CORRECT; only its § 4 leg is mis-anchored. CF-83's Clause 1b must name threshold AND metric explicitly. Reported, not resolved.

### Item 13 — T1 HYPOTHESIS REFUTED, no catch (Rule 29 — no artifact claimed the coverage)
`run_start` has a single emit site (`packages/sim/src/run/state.ts` RunController constructor); the `replaySameClass` path (`apps/client/src/run/useRun.ts`) supplies both `classId` and `startingRelicId` and returns early via `resetRun()` when `starter` is null — so it cannot drop either field. The `replaySameClass` cause is REFUTED, not routed. The 2/28 join gap stays open as an unexplained observation. Rule 29 — no artifact claimed this join coverage, so no catch.

### Item 14 — GHOST-GENERATOR DUPLICATION: constraint recorded, NO CF OPENED
`makeGhostForRound` (`apps/client`) vs `generateProceduralGhost` (`packages/sim/test`) is M1.3.4a's ratified delete-at-M2 scaffolding; it overlaps open CF-13 and CF-16 and attaches to them as a constraint. No new CF.

### Item 15 — CF-86 OPENED — class-asymmetric round-1 resolution
Highest existing CF at tip `38007f7` is CF-85 (grep-walked live); next is CF-86. CF-86 scope: the Tinker side of item 5's decomposition — the ~36-point stall residual above the zero-resolver floor. First instrument: a round-1 purchase-by-class query against existing PostHog data. May ride CF-83's implementation rather than queue separately if the shared-sim ramp closes it. OPEN.

### Item 16 — CITATION-ANCHOR MIS-ATTRIBUTION in a hand-off (Catch 80 + Drift 59; Claude Code vs master-dev, master-dev ruled it counts)
A master-dev hand-off INPUTS block located POPULATION CAVEAT (iii) in the `2125b82` entry; it lives in the CF-64 close entry at `38007f7`. Asserted unqualified and undisclaimed, so the Rule 29 disclaimer exclusion does not apply. NOT clerical under Rule 15: had it landed unchecked, the entry would withdraw a caveat against an entry that never contained it, and a date+header grep would return nothing — mis-directing, not mislabelling. Class A / Rule 30, on the qualifier axis (the caveat's substance is correct; only its location anchor was wrong). Caught by Claude Code against master-dev, which master-dev ruled counts the same as an implementation catch.

### Process — catches, drifts, held candidates, closing observation
- **Catches 76–80 / Drifts 55–59 (Class A, Rule 30; each a positive factual claim checkable against available ground truth at authoring, emitted without the check; one Topic-2 drift paired to each master-dev-authored catch per the Catch 66 / Drift 46 precedent):** 76+55 = item 1 (pool-mean universal claim, landed in canon); 77+56 = item 2 (fabricated path, landed in canon); 78+57 = item 4 (class-asymmetry mis-attribution, hand-off refuted pre-canon); 79+58 = item 7 (byte-delta direction inference, landed in canon); 80+59 = item 16 (citation-anchor mis-attribution, hand-off refuted pre-canon). Items 4 and 16 are hand-off claims caught before reaching canon — a halt-gate correction still counts (Catch 64 / Catch 66 precedent).
- **Closing observation (mints nothing):** all five catches are master-dev claim-accuracy failures and zero are shipped defects — the pattern the standing concern (process apparatus outgrowing playable surface) names.
- **Rule 28 amendment candidate — HELD, NOT CODIFIED (no new ordinal):** items 1 and 7 share a shape — a claim emitted without querying the segment that would falsify it (item 1: a universal claim from a pool mean without the round-1 segmentation; item 7: a direction claim from a whole-replay byte delta without the per-action-type breakdown) — and item 7 is its second instance. Mechanism stated: a claim of universal or directional form over an aggregate must be checked against the finest available segmentation of the data it generalizes over before it is emitted. Codify on a further instance. Deliberate restraint per the standing concern.
- **Uncounted convention (recorded, not codified; no ordinal):** a superseded hand-off prompt left live in a chat thread is a carried-state hazard of Rule 32's own class — mark superseded prompts VOID at the head. One line, no recurring burden.

### Counter
Ordinal walk (live from canon at tip `38007f7`): highest Catch 75 (69–75 contiguous), Drift 54 (50–54 contiguous), CF-85, Pattern 10 — no phantom higher ordinals. Baseline: main tip `38007f7`; governing entry decision-log.md 2026-07-19 § "CF-64 CLOSED …" — 75/32/10/54/50 → **80/32/10/59/51** — catches **80** / rules **32** / patterns **10** / drifts **59** / open-CFs **51**. Delta: catches **+5** (76 item 1, 77 item 2, 78 item 4, 79 item 7, 80 item 16); rules **+0** (Rule 28 amendment HELD, no ordinal); patterns **+0** (Pattern 10 further instance, referenced); drifts **+5** (55–59, one Topic-2 drift paired to each catch); open-CFs **+1** (CF-86 opened, item 15; none closed — CF-84 gains a real-play instance, item 10). Docs-only supersession — no PR, no merge, no code.
...
### FINDING 1 — combat non-resolution (structural, not tuning)
- **Rate.** Frozen corpus: 391/1096 combats (35.68%) reach MAX_COMBAT_TICKS=600 as timeout draws — the telemetry-plan.md § 4 tick-cap metric (`endedAtTick == 600`). Real play (PostHog `combat_end`, n=260): 218 player_win (83.8%, avg_tick 133), 35 draw (13.5%, avg_tick 579), 7 ghost_win (2.7%, avg_tick 304). Guardrail is <1% (balance-bible.md § 2; telemetry-plan.md § 4) — corpus 35.7×, real play 13.5×.
- **Mechanism = thin early bags.** 85% of stalls fall in rounds 1–3 (331/391); the round-11 boss stalls 0/29. Stalls average 0.10 net DPS vs 2.65 for resolvers; healing absorbs 79% of gross offense. By inclusion-exclusion on the Phase-1b HP-retained buckets (player 291 + ghost 297 at 100% HP, of 391), ≥50.4% of stalls exchange zero net damage on BOTH sides (291+297−391 = 197; 197/391 = 50.4%).
- **§ 5 cause REFUTED.** telemetry-plan.md § 5 predicts a "2-armor-stacked stalemate." Refuted three ways: (a) no damage-reduction mechanic exists in `packages/sim` (grep of src for reduc/armor/mitigat/block/defense = empty); (b) ghost HP ignores its bag's maxHpBonus (`packages/sim/src/combat/ghost.ts` `makeGhostForRound` — round-scaled `BASE_COMBATANT_HP + floor((round-1)/2)*2` only); (c) both-sides-2+-maxHpBonus-items = 0.9% of stalls (3/342 reliable).
- **Structural-by-construction.** Round-1 income (4g, gdd.md § 2) buys one 3g common. At balance-bible.md § 1's stated 0.5 damage-equivalent/s common budget, one item closes a 30-HP ghost in exactly 60s = the cap. Measured per-item gross output is 0.27/s → 111s → round 1 CANNOT resolve within the cap by construction. The fix is structural, not a damage-number tune.
- **Draw renders as loss.** A timeout draw collapses to roundOutcome 'loss' (`packages/sim/src/run/state.ts` `applyCombatOutcome`: `input.outcome === 'player_win' ? 'win' : 'loss'`) and renders "LOST" (`apps/client/src/combat/CombatOverlay.tsx`, `isWin = outcome === 'player_win'`). balance-bible.md § 2's "both sides take a heart loss" is vacuous — the ghost has no heart economy in the run model.
- **Draw-path split (CONFIRMED; scoped by CF-84).** Draws arrive by TWO paths, verified in code: `packages/sim/src/combat.ts` `simulateCombat` emits 'draw' at the in-tick death check when both combatants reach ≤0 HP on the same tick (the mutual-KO path), SEPARATELY from the tick-cap timeout draw. PostHog real-play draws average tick 579 (outcome='draw', n=35) — below the 600 cap — reflecting the mix. The frozen corpus exercises the mutual-KO path 5 times (tick<600 draws, ticks 60–250; C3 grep), alongside 391 timeout draws (tick==600) = 396 total corpus draws. Recorded here; disambiguation carried by CF-84.

### FINDING 2 — combat legibility (Phase 1a client-surface enumeration)
The core mechanic is illegible to the player: item_trigger renders a nameless positional flash at the source anchor with no item name (`apps/client/src/combat/CombatScene.ts`; `anchorResolution.ts` ANCHOR_RULE item_trigger='source'); `describeItem` is disabled during combat (`apps/client/src/bag/DraggableItem.tsx`); the ghost has no rendered bag (`apps/client/src/bag/layout.ts` — ghost.itemAnchors intentionally empty); the gdd.md § 2 pre-round intent hint is hardcoded ("Ghost / Round 4 · ±1 trophy" + fixed iron-sword/wooden-shield silhouette on every round regardless of opponent; `apps/client/src/hud/LeftRail.tsx` `OpponentSilhouettes`); "view opponent build" (gdd.md § 2 resolution) is unbuilt (`DraggableItem.tsx` names it as such); the replay log is round-outcome-only with a parity-guessed opponent class (`apps/client/src/screens/mobile/tabs/LogTab.tsx` `opponentClassLabel`).

### the 2026-07-12 revisit trigger FIRED (early)
decision-log.md 2026-07-12 § "M1-exit-gate testing path: solo self-cert (roadmap.md amended)" logged as accepted risk that solo self-cert would not surface fresh-player confusion, to be revisited before the M2 exit gate. The trigger fired early and is recorded. Self-cert basis (2026-07-19): 5 runs, both classes, 4–5 min each, 44 round wins / 6 losses, zero crashes, an identical confusion-and-boredom note from the DEVELOPER on all five.

### DISPOSITIONS (ratified)
- **CF-64 — two-stage regeneration (CF-64 STAYS OPEN, closes at Stage 1 merge).** Stage 1 regenerates on UNCHANGED code, isolating the ~41-file frozen-corpus drift (decision-log.md 2026-07-11 § "CF 58 CLOSED …") in its own commit. Stage 2 regenerates after the balance change. Rationale: the generator is deterministic BY CONSTRUCTION (Phase 1b C2 — `packages/sim/test/determinism/generate.ts` has no Date/random/env; `generatedAt` hardcoded; `createRng(spec.seed)`; write path `writeAllFixtures` wipe-and-rewrite), so the frozen corpus guards a property better tested by regenerate-twice-and-compare than by freezing.
- **Ordering (ratified): CF-64 Stage 1 PRECEDES CF-83.** A balance change landing before the drift is isolated produces a diff mixing the intended change with pre-existing drift — the specific condition the two-stage disposition exists to prevent.
- **Fix direction (ratified):** invert the ghost HP curve (`makeGhostForRound`) so early-round HP tracks available offense; add a post-tick-~300 damage ramp as a construction-level resolution guarantee. REJECTED and recorded: raising item damage (moves an equilibrium without bounding it), shortening the cap alone (no fix), granting the player base damage (contradicts balance-bible.md § 2's all-damage-from-items).
- **Recorded, not actioned:** resolvers average 21.1s against balance-bible.md § 1's 12s calibration basis — the power budget is loose. M2 balance pass, not this fix.

### CFs OPENED (ordinals walked from canon; highest existing = CF-82 per grep at tip 66dfb36)
- **CF-83 — combat non-resolution:** ghost HP curve inversion + post-tick damage ramp (the FINDING 1 fix direction). OPEN.
- **CF-84 — draw semantics:** separate the timeout draw from the mutual-KO draw; stop rendering a timeout as a loss. Corpus coverage confirmed — 5 sub-cap mutual-KO draws (C3) plus 391 timeouts; real-play draw avg_tick 579 (PostHog) reflects the mix. OPEN.
- **CF-85 — combat legibility:** item attribution on trigger, opponent build inspectable, real runtime intent hint. Absorbs the hardcoded-placeholder finding (FINDING 2). OPEN.

### SEQUENCING (ratified — supersedes the uncommitted self-cert-first inversion)
Combat resolution (CF-83/84/85) PRECEDES CF-68 PR-B. Rationale: PR-B ships a daily-contract streak — a reason to return — to a loop whose core mechanic reads as illegible and inconsequential to its own developer. CF-68 PR-B remains OPEN and unstarted. This supersedes the prior uncommitted self-cert-before-PR-B sequencing, which never received a SHA (memory-only, corrected 2026-07-19; M1 exit gate already CLOSED per decision-log.md 2026-07-13 § "M1 EXIT GATE CLOSED").

### PROCESS
- **DoD amendment — migration-applied gate (no ordinal).** A PR shipping a migration is NOT done until the migration is applied to the dev database AND the enabled path is exercised once against real play or a real request. CI green against an ephemeral service container does NOT discharge it. Basis: migrations 0002 and 0003 were merged, gated, and closed in canon while unapplied to the dev DB; Session-1 real play produced 100× HTTP 503 on `/v1/player/save` (`DrizzleQueryError: relation "applied_round_results" does not exist`). CF-77's delta-trophy push and CF-68 PR-A's participation write both executed under real play for the FIRST time on 2026-07-19 (13/13 `/v1/player/save` → 200 after the pending migrations were applied).
- **Catches / drifts (Class A, Rule 30 as re-amended; master-dev-authored unless noted; drifts paired per the Catch 66 / Drift 46 precedent):**
  - **Catch 69 + Drift 50 (session brief):** the closing-entry date cited 2026-07-19; actual header 2026-07-18. Disposition range cited D1–D7; actual D1–D4.
  - **Catch 70 + Drift 51 (master-dev):** asserted `acbea74` as the PA1–PA7 landing commit by inference from a baseline citation. Refuted — `51feb15` is the PA1–PA7 append (decision-log.md 2026-07-18 § "CF-68 PR-A Phase 1 dispositions RATIFIED …"); `acbea74` is a chore removal of `PLAN-cf54-client-version.md`. The paired non-verdict on `51feb15` takes NO catch (explicitly disclaimed by the issuing prompt — Rule 29).
  - **Catch 71 (Claude Code; NO paired drift, per the Catch 68 author-agnostic precedent):** rendered the ratification chain with `acbea74` inserted and its direction reversed, attributed to the brief, which does not contain it.
  - **Catch 72 + Drift 52 (session brief):** three false claims about M1-exit-gate state — decision pending, stale criteria, "never measured." All three refuted by decision-log.md 2026-07-12 § "M1-exit-gate testing path" and 2026-07-13 § "M1 EXIT GATE CLOSED"; the gate CLOSED 2026-07-13.
  - **Catch 73 + Drift 53 (master-dev):** propagated Catch 72 across four turns and built a play protocol and a pre-committed decision rule on it, with decision-log.md in-project and searched for other fields in the same session. RULED DISTINCT from Catch 72 (deciding reason: Rule 32 is codified off this failure — merged, the new rule has no instance to point at and its antidote collapses into Rule 30's; Catch 72 is "a false thing was written," Catch 73 is "the reader had ground truth in hand, searched it for other fields, and never checked this one").
  - **Catch 74 (Claude Code; Class A, Rule 30 re-amended; NO paired drift, per the Catch 68 / 71 author-agnostic precedent):** the halted draft's provenance flag and its FINDING-1 anomaly bullet both attributed the mean-579 tick to CORPUS data; it is PostHog REAL-PLAY data (draw avg_tick, n=35 — corpus draws sit at ~600). A provenance flag failed on the one attribution it existed to make (Rule 29 — stated job, failed). Corrected at both sites in this revision.
- **Rule 30 RE-AMENDED (extends; NO ordinal; mechanism-first per Pattern 10's ratified remedy):** "Never emit a factual claim whose truth is checkable against any available ground truth — repository artifact (code, test, schema, migration, decision-log.md), tool output, or prior message in the session — without having checked it, in any artifact headed for canon or for a hand-off. Verify from ground truth, or explicitly disclaim authority and instruct resolution." Extends the prior "unverified shipped-state claim" wording to any ground-truth-checkable factual claim (including cross-tool / transcript-checkable ones, e.g. Catch 74).
- **Rule 32 NEW — carried-state reconciliation.** At session start, before any planning rests on it, every claim about milestone state, gate status, or open decisions in ANY carried-in state artifact — chat brief, tool topic file, skill file, prior-session summary — is verified against decision-log.md. The counter alone does not discharge it. A blanket "the log wins" clause establishes precedence, not verification. Codification basis (bend-note, OBSERVED): the M1-exit-gate stale premise had TWO independent carriers this session — the chat brief AND project_m2_position.md — two propagation vectors across two tools in one session; the generic shape is observed, not asserted. Discipline is low-burden (verify carried claims at session start); the surface recurs every session. Instances: Catch 72 / Catch 73.
- **Citation-discipline amendment (no ordinal):** a baseline citation's tip SHA and its section anchor are INDEPENDENT referents; state them separately — `Baseline: main tip <SHA>; governing entry decision-log.md <date> § "<header>" (landed <SHA>)`.
- **Watch item — SECOND instance (folds in, no ordinal):** self-authored documents fail in the middle, not at the top. Instance 1 — a stale "NEXT:" header caught by an adversarial sweep. Instance 2 — project_m2_position.md's stale "branches NOT deleted" line, found incidentally rather than by the correction sweep. A THIRD instance revisits codification.
- **PAT capability correction (recorded):** canon's "the PAT cannot delete refs" is accurate for the fine-grained `PACKBREAKER_GH_PAT` only. The GCM-cached CLASSIC PAT DOES delete refs via `git push origin --delete`; both merged branches (`m2.1-cf77-pr2-client-producer`, `m2.1-cf68-pra-server`) were deleted 2026-07-19.
- **Pattern 9, further instance (recorded, no ordinal):** `error_boundary_caught` is typed, Zod-validated, and test-covered with ZERO production emit site. Rule 29 no-catch — telemetry-plan.md § 9 says "recommend adding," not "shipped."

### Counter
Baseline (tip `66dfb36`, decision-log.md 2026-07-18 § "CF-68 PR-A CLOSED …") 68/31/10/49/48 → **74/32/10/53/51** — catches **74** / rules **32** / patterns **10** / drifts **53** / open-CFs **51**. Delta: catches **+6** (69 session-brief date/range; 70 acbea74-assertion; 71 Claude-Code chain-render; 72 M1-gate false claims; 73 propagation-and-build; 74 Claude-Code 579-provenance); rules **+1** (Rule 32 — carried-state reconciliation; Rule 30 re-amendment + citation-discipline amendment are amendments, no ordinal); patterns **+0** (Pattern 9 further instance + Pattern 10 remedy referenced, no ordinal); drifts **+4** (50/51/52/53 — the four master-dev catches; Catches 71 and 74 are Claude-Code, no paired drift); open-CFs **+3** (CF-83/84/85 opened; CF-64 and CF-68 stay OPEN, none closed). Rule 20 gate entry — no PR, no merge, no code.

---

## 2026-07-18 — CF-68 PR-A CLOSED (server daily-participation MECHANISM: daily_participation table + write-DTO tolerance + streak derivation relocated into the store transaction — CF-68 AMENDED's "verified daily-completion event" is REVISED, not met, and transfers to CF-79); CF-68 stays OPEN (PR-B pending)

### Merge
PR \#52 (`m2.1-cf68-pra-server`) merged into `main` via `--no-ff` at merge commit `169e88ccafdd6881a4f01389fb9d3aa80bf7267a` (merged_at 2026-07-19T14:14:05Z — UTC commit-day; decision-day 2026-07-18); two parents `732434c` (main) + `b9daefa7ff94f380faf41dc7ca26f2cbbcb0610b` (feature), pushed `732434c..169e88c`; PR \#52 auto-closed server-side (`merged:true`, `merge_commit_sha 169e88c`). Branch `m2.1-cf68-pra-server` NOT deleted (PAT cannot delete refs — manual/local). Implements the CF-68 PR-A dispositions as ratified — decision-log.md 2026-07-18 § "CF-68 PR-A Phase 1 dispositions RATIFIED …" (PA1–PA7), § "CF-68 PR-A dispositions AMENDED …" (PA8–PA11), § "CF-68 PR-A test-topology dispositions RATIFIED …" (TT1–TT4). Server-only + the two byte-identical `content-schemas.ts` DTO mirrors. **CF-68 does NOT close here** — PR-B (client daily-fetch leg) remains; closure lands at the PR-B merge per the sub-PR convention.

### What shipped (substance already in canon; not re-recorded here)
Migration `0003_daily_participation` (PK `(account_id, daily_date)`, 0002's conventions, 0002 fenced — PA5/PA11); `lastDailyAttempted` → `.nullable().optional()` accepted-and-unread + `dailyContractId`/`dailyDate` `.optional()`, `.strict()` retained (PA1/PA2/PA8); server-date gate removed (PA9); `deriveDailyStreak`+`previousDay` relocated route→store (byte-unchanged body, no import cycle — PA7/TT1); participation written + streak derived inside `applyRoundResult`'s transaction under the existing `FOR UPDATE` lock, PA3 invariant enforced (a neutral/mismatched round writes trophies only).

### daily-identity equality lives in the TRANSACTION, not the Zod layer (recorded per PA6)
`dailyDate` is validated at the Zod layer as a well-formed ISO date ONLY. The `dailyDate === serverToday` (and `dailyContractId ===` the server's daily contract id) equality check is performed INSIDE `applyRoundResult`'s transaction, against a `buildDailyContract`-re-derived server truth the route forwards. A Zod-layer equality check would 400 a stale identity — reintroducing exactly what PA9 removed; PA6 requires a SILENT skip with the round push succeeding.

### Rule 28 — four falsifications (break → fail → revert byte-clean; verbatim)
- **F1 (PA3 non-daily guard):** removed → realsql scenario m fails `expected +0 to be 1` (a neutral round zeroed a streak of 1). CONFIRMED.
- **F2 (PA5 ON CONFLICT DO NOTHING):** removed → realsql scenario k fails `duplicate key value violates unique constraint "daily_participation_account_id_daily_date_pk"` (code 23505) — a round-2 push 500s. CONFIRMED.
- **F3 (PA4 lock-then-derive ordering): NOT shipped — because the invariant is NOT LOAD-BEARING today, not because a test was hard to write.** MECHANISM — the daily derivation is CONVERGENT under concurrency: two racing same-day pushes both read `prevLastDailyAttempted = yesterday`, both compute `prev + 1`, and both write the SAME value; the sequential interleaving instead hits the same-day guard (`prevLastDailyAttempted === serverToday → return prevDailyStreak`) and writes that SAME value. A lost update is INDISTINGUISHABLE from a serialized one because the per-day advance is IDEMPOTENT BY CONSTRUCTION. Empirically demonstrated: under a derive-before-lock break, all 19 shipped realsql tests PLUS a TEMPORARY concurrent same-day probe (20 it-blocks in that one run) pass — the probe was reverted and is NOT in the shipped suite (which stays at 19). The `FOR UPDATE` lock is NOT guarding the daily leg — it guards the TROPHY delta, and the pre-existing realsql scenario i already falsifies THAT. Shipping an F3 test would reproduce Catch 65's shape EXACTLY (an ordering test that passes under a derive-before-lock break) — this is the FIRST PRODUCTIVE FIRING of Rule 28's halt branch. Rule 29 no-catch, SIXTH application: the issuing prompt explicitly disclaimed F3 as the live risk and named a halt as the correct outcome.
- **F4 (hostile-payload):** wire the client `lastDailyAttempted` back into the route's forwarded daily identity → route hostile-payload test fails `expected 4 to be 3`. CONFIRMED.

### Tests — net +8 (+14 added / −6 deleted)
Unit `deriveDailyStreak.test.ts` **+6 over deriveDailyStreak's FIVE branches** — yesterday→+1 (branch 4), today→unchanged (branch 3), gap→reset-to-1 (branch 5 / default), null-attempt→0 (branch 1, retained-dead), attempt≠serverToday→prev (branch 2, retained-dead), AND a SIXTH test **never-attempted (prev null)→1** — a second exercise of the reset-to-1 default (branch 5) via a distinct input (prev null vs prev a stale date), the two inputs that both fall through to `return 1`. Realsql 14→19 (+5, daily scenarios j–n). Route 25→22 (−6: EXPLOIT-REGRESSION + `server-derived dailyStreak` block deleted per TT2; +3: hostile-payload / pass-through / field-absent; the two gate tests inverted+renamed in place). Server suite 138→146 (realsql live against Neon). F3 shipped 0 tests, as ruled.

### Gates
`tsc -b --noEmit --force` (apps/client, load-bearing for the shared-DTO change per amended Rule 2/3) exit 0; server `tsc --noEmit` + `eslint --max-warnings 0` clean; `check-schemas-sync` OK; Rule 13 triple-green — `turbo lint typecheck test build --force` ×3 cold-cache, 25/25 tasks each (0 cached), server 146/146 each.

### CF-82 AMENDED — the convergence trap (F3 becomes load-bearing if the derivation stops being idempotent)
CF-82 (strict `lastDailyAttempted` removal + retained-dead-branch disposition) gains: if the daily derivation's shape ever changes so the per-day write is NO LONGER IDEMPOTENT — counting total dailies, deriving from participation-row COUNTS, or ANY read of a value it also increments — then the convergence that makes F3 unfalsifiable today disappears, lock-ordering becomes LOAD-BEARING with NO test guarding it, and **F3 MUST be re-attempted as a deterministic falsification BEFORE that change ships.** CF-82 stays OPEN; no open-CF delta; no new ordinal.

### Catch 68 (Class A — Rule 30, third instance this session) + Rule 10 category 3 WIDENED
**Catch 68 (Class A — change-site contradiction; RULE 30 as amended).** The held PR body's F3 line claimed "all 20 realsql tests …" — a FALSE claim about shipped state: the realsql suite is 19 it-blocks (the "20" counted 19 shipped + a since-reverted temporary probe), refuted AT THE CHANGE SITE by the realsql file (19 it-blocks, 0 `PROBE`, grep-verified) and contradicting the body's own Layer B "19." Attributed to **Rule 30 (as amended)** — "never emit an unverified factual claim about shipped state … a claim about what tests exist … in any artifact headed for canon or for a hand-off." **THIRD Rule 30 instance this session**, after Catch 66 (fabricated `accounts.` table qualifier) and Catch 67 (false "deriveDailyStreak has unit tests" claim). Corrected before the PR was created. **NOT Class B** — the earlier "handoff-verify failed Cat 3" framing was wrong: Cat 3 executed its stated job (under its then-current wording the summary totals DID reconcile); the gap was the check's SCOPE, not a failure, and is addressed by the widening below. The defect is the FALSE CLAIM itself.

**Author-agnostic (recorded).** Rule 30's amended text names no author ("any artifact headed for canon or for a hand-off"). Catches 64 / 66 / 67 were all master-dev's; **Catch 68 is the FIRST Rule 30 instance with Claude Code as author** — confirming the rule is author-agnostic AS WRITTEN, no amendment needed. **This is Pattern 10's remedy APPLIED and PASSED:** the wording, scoped to the MECHANISM (any unverified shipped-state claim) rather than a surface feature, caught a variant it had never seen — a test-count claim, by a different author. REINFORCING instance for Pattern 10; codify nothing new.

**NO drift pairing.** Drifts 45–49 are all master-dev deviations; Catch 68 is Claude Code's and takes no paired drift.

**Rule 10 category 3 WIDENED (amendment — NO new ordinal).** Independently: Cat 3 now reads — EVERY numeric claim anywhere in the artifact must reconcile against that artifact's own enumeration, not only the summary totals. Cat 3 as previously written EXECUTED correctly (the totals reconciled); its SCOPE was the surface feature (summary totals) rather than the generative mechanism (any number derived from a countable set) — **Pattern 10's shape** (codification-to-surface-feature; decision-log.md 2026-07-18 § "CF-68 PR-A test-topology dispositions RATIFIED"), recurring one session after Pattern 10's codification. Widened Cat 3 re-run on the corrected body: all five categories PASS.

### Codex — round 1: 1 P1, dispositioned as a CF-79-bound residual (no catch, CF-67)
Round 1 (review `4729813149`, sha `b9daefa`, fresh top-level `@codex review` trigger): 1 finding, **P1 — "Require non-forgeable evidence before crediting daily streaks."** ACCEPTED as accurate against code — the store matches the client-supplied `(dailyContractId, dailyDate)` against the server's daily identity, but both are PUBLIC (`daily-placeholder` is a constant; the date is public), so an authenticated caller can echo them on a forged round push and forge participation. Dispositioned as a **CF-79-bound residual** (subsumption, below), not fixed. **NO catch (CF-67)** — a Codex pre-merge finding dispositioned before merge is the review WORKING; the clearest instance of that ruling to date.

### CF-68 AMENDED — REVISED (reading ii: "verify a real attempt" is unsatisfiable pre-CF-79)
The amendment (decision-log.md 2026-07-16 § "CF-75 + CF-76 CLOSED …") reads, verbatim: "CF-68 (daily-contract client-fetch leg) closure MUST land a **server-side participation-evidence mechanism** (a verified daily-completion event) alongside the client-fetch wiring. Wiring CF-68 alone — making the client honestly assert today's date without the server able to verify a real attempt — ACTIVATES CF-76's bounded gap rather than closing it." Its operative words — "a verified daily-completion event" and "the server able to verify a REAL attempt" — demand NON-FORGEABLE verification (reading ii). PR-A does not deliver it (Codex round 1) and CANNOT (any evidence the client carries and echoes is forgeable). **REVISED** to the achievable pre-CF-79 posture: PR-A lands the server-side participation MECHANISM (server-written rows, server-derived streak, under its own lock) and SUBSUMES daily forgery into the existing round-push forgery; the "verified completion / non-forgeable" property transfers to CF-79's replay-validate. The unreachability of the original wording before CF-79 is the stated reason for the revision.

### D3 security-claim CORRECTED (subsumption, not elimination)
D3 (decision-log.md 2026-07-18 § "CF-68 next-target selection + Phase 1 dispositions RATIFIED …") and the PR body claimed "CF-76's exploit ceases to exist as a class." STRICKEN — the mechanism is right, the security conclusion was wrong. Accurate framing (now in the PR body Trust model):
- PR-A does NOT make daily participation non-forgeable and cannot; both matched values are public, so an authenticated caller can attach them to a forged round push.
- SUBSUMPTION: daily-streak forgery is no longer an independent, cheaper attack surface — it requires the SAME forged round push that already yields forged trophies (already named, accepted, CF-79-bound). Two residuals collapse into one, closed by one fix.
- The BOUND IS UNCHANGED: CF-76's post-gate posture was +1 streak/day; the `daily_participation` PK bounds the forgery to +1/day. PA9's gate removal loosened nothing.
- Independent of forgery, the streak is now server-derived from rows the server wrote, under its own lock, inside the transaction — a real improvement that stands.

### Option (b) REJECTED and recorded (a client-carried token is security theater)
A server-issued per-run daily token proves the client FETCHED the daily, not that it PLAYED it: a forger authenticates, fetches legitimately, and echoes the token on a forged push — the floor rises by one HTTP request. Any evidence the client carries and echoes is forgeable by whoever can obtain it. Non-forgeability requires server-side re-simulation (CF-79) and nothing short of it. Shipping a token that reads as a defense while not being one is exactly what PA10 ratified against.

### CF-79 AMENDED — daily-participation forgery residual added to scope
CF-79 (replay-validate) now carries the daily-participation forgery residual ALONGSIDE the daily-credit non-reconciliation it already holds (decision-log.md 2026-07-18 § "CF-68 PR-A dispositions AMENDED …"). Replay-validate closes the trophy residual AND the daily residual in ONE stroke — the payoff of the subsumption argument. CF-79 stays OPEN; no open-CF delta; no new ordinal.

### CF-68 closure position (recorded)
CF-68 still closes at PR-B's merge under the sub-PR convention, with the forgery residual transferred to CF-79. Rationale (recorded): holding a CF open for a property only a LATER CF can deliver is CF-hoarding.

### Drift 49 (NEW, Topic 2) — overstated security claim ratified into canon
Master-dev ratified an OVERSTATED SECURITY CLAIM into canon: D3's "CF-76's exploit ceases to exist as a class" — a WRONG security conclusion drawn from a CORRECT mechanism. Distinct in shape from Catch 66 / 67 (unverified claims about SHIPPED STATE — a table qualifier, a test topology); this is a sound mechanism with an unsound security CONCLUSION, and it is deliberately NOT folded into Rule 30 (which governs unverified shipped-state citations, not security reasoning). Topic 2, +1 drift. No catch — the mechanism shipped correctly; the error was the conclusion, caught by Codex pre-merge (CF-67).

**Error chain (recorded honestly).** D3 asserted CF-76's exploit ceased as a class → Codex round 1 REFUTED it pre-merge (the identity match is against public values) → the amendment (CF-68 AMENDED) that should have caught the overstatement was ITSELF unachievable when written, demanding "a verified daily-completion event" that non-forgeability makes unreachable before CF-79's re-simulation. Drift 49 covers the overstated D3 claim; CF-67 governs the Codex finding (no catch — the review working); the amendment is REVISED (above).

### Relay-discipline — SECOND instance (watched, NO ordinal)
The standing relay discipline ("a silent pass is indistinguishable from a skipped gate") fired here on a DETERMINATION, not a gate: the Catch 68 error-vs-ambiguity determination — made when Catch 68 was recorded — was not re-relayed as a clear CALL when the counter question arose (it was surfaced as an open "master-dev to confirm" rather than a stated disposition), so master-dev could not rule and had to prompt for it. NO new ordinal — the existing discipline covers it. But its wording leans toward GATES / PASSES, not determinations / conclusions — Pattern 10's shape (a discipline scoped to its surface feature, not its generative mechanism = any consequential conclusion that must be surfaced). WATCHED, not codified; a THIRD instance revisits whether to widen the wording.

### Counter
Baseline (tip `732434c`) 67/31/10/48/48 → **68/31/10/49/48** — labelled catches **68** / rules **31** / patterns **10** / drifts **49** / open-CFs **48** (⚠ drifts=49 and open-CFs=48 — DO NOT transpose). Delta since tip: catches **+1** (Catch 68 — Class A, Rule 30, third instance this session); drifts **+1** (Drift 49 — Topic 2, overstated security claim). Rules / patterns / open-CFs unchanged — the Rule 10 Cat 3 widening + CF-68 AMENDED revision + CF-79 / CF-82 amendments are all AMENDMENTS (no ordinal); Pattern 10 REFERENCED; the Codex P1 is no-catch (CF-67); F3's halt is Rule 29 no-catch (SIXTH application). CF-68 stays OPEN (closure at PR-B).
...
### 1 — CF-68 next-target selection, RATIFIED

CF-68 (daily-contract client-fetch leg, AMENDED) is the next target, selected following CF-77's close at `e7c3a34`; the CF-77 sequencing blocker is resolved. Chosen over CF-78 (synced-toast primitive), CF-38 (gold-axis display co-drift), and CF-80 (account→run seed) on the ground that CF-68 is the only open backlog item carrying a LIVE amendment (decision-log.md 2026-07-16 § "CF-75 + CF-76 CLOSED …", "CF-68 AMENDED") against an EXPLOITABLE bounded posture (CF-76 — bounded design ratified `9d5ab01`, decision-log.md 2026-07-16 § "CF-75 + CF-76 Phase 1 RATIFIED"). Folded into this selection entry per the precedent at decision-log.md 2026-05-26 § "M1.5c next-target selection".

### 2 — CF-68 Phase 1 dispositions, RATIFIED

**D3 — the evidence mechanism (load-bearing).** Participation-evidence is NOT "validate the client's daily claim." `lastDailyAttempted` is REMOVED from the PUT write-request DTO (`PlayerSaveWriteRequest`, content-schemas.ts); the client never asserts a daily attempt. The server writes participation as a SIDE-EFFECT of an accepted round push carried under a daily-bearing `runId`, and derives the streak from rows IT wrote itself. This is the CF-77 move one field over — the client stopped sending `trophies` and the server derived the delta (decision-log.md 2026-07-17 § "CF-77 Phase 1 RATIFIED …"); here the client stops sending the daily claim and the server derives participation. Consequence: CF-76's exploit CEASES TO EXIST AS A CLASS — there is no client-asserted daily field left to forge — rather than being bounded further. What remains is the generic forge-a-round-push residual already named and CF-79-bound (decision-log.md 2026-07-17 § "CF-77 Phase 1 RATIFIED …", Round-3 disposition), de-amplified by the `MAX_ROUND` cap (`MAX_ROUND`, validation/playerSave.ts).

- **Correctness, not security (recorded plainly).** `dailyContractId` + `dailyDate` on the payload exist for CORRECTNESS — establishing WHICH calendar day a run belongs to so the streak credits the right day — NOT for security. The daily seed (`DailyContractResponse.seed`, content-schemas.ts) is deliberately NOT echoed on the wire: it adds nothing over the round-push forgery floor, and echoing it would imply an RNG coupling that CF-81 (§ 8) has not ratified. A future reader must not mistake these two fields for a defense.
- **Semantic tightening.** *attempted* := at least one ACCEPTED round result under a daily-bearing `runId`. A daily abandoned before round 1 resolves earns nothing (no accepted round push → no participation row).
- **Storage.** A dedicated `daily_participation` table, PK `(account_id, daily_date)`. Two reasons, both recorded: (i) the natural key `(account_id, daily_date)` differs from the per-round idempotency key `(account_id, run_id, round)` of `applied_round_results` (migration `0002`); (ii) `applied_round_results` is a just-proven CF-77 surface and stays FENCED — the daily leg does not widen it.
- **Streak-state anchor.** `deriveDailyStreak` (`deriveDailyStreak`, routes/playerSave.ts) branch logic — yesterday → +1 / today → unchanged / gap → reset to 1 / null → 0 — is RETAINED, now fed from server-derived participation. The streak-state anchor is `player_saves.last_daily_attempted` (`playerSaves`, db/schema.ts), server-written. Relocation to `accounts` was CONSIDERED and REJECTED: `accounts` is identity-only (`accounts`, db/schema.ts — id / clerk_user_id / anon_id_at_signup / created_at; decision-log.md 2026-07-13 § "M2.1 PR1 CLOSED …"), and gameplay state stays on `player_saves`. PR-A owns NO relocation migration.
- **Write-direction-only removal (precision).** The DTO removal is WRITE-DIRECTION ONLY. `lastDailyAttempted` is removed from the PUT write-request DTO; the `player_saves.last_daily_attempted` column REMAINS, server-written, and REMAINS on the GET read/hydration path (`PlayerSaveResponse`, content-schemas.ts). PR-A must NOT drop the column or the read.

**D4 — PR split.** PR-A (server: `daily_participation` table + migration, DTO field removal + `dailyContractId`/`dailyDate` addition, participation write + streak derivation from server rows, real-SQL test) ≈ 2.5–3.5 d → PR-B (client: production `GET /v1/contract/daily` caller, run the daily contract, fold the participation signal into the existing per-round push) ≈ 1.5–2 d. CF-68 CLOSES at PR-B's merge (sub-PR convention: closing entries held until a real merge SHA exists).

### 3 — Rule 29 (NEW): no-artifact-claimed gaps take no catch

**Rule 29.** A verification artifact that fails its STATED job takes a catch; a gap found at a review or halt gate that NO artifact claimed does not. **Instance 1** (held candidate, codification-eligible): the CF-77 arc's commit-9 identity-guarded shift — the mid-drain-discard × positional-shift interaction was found at the DESIGNED master-dev review gate, and NO verification artifact had claimed coverage of it, so it took no catch (decision-log.md 2026-07-18 § "CF-77 CLOSED — Phase 2 PR2 …", the commit-9 HELD-candidate line). **Instance 2** (this arc): the carried code-site line reference in this session's Phase 1 INPUTS block drifted (a `useRun.ts` `createRun` call-site line cited from a prior session had shifted), but the INPUTS block EXPLICITLY disclaimed authority over line numbers and instructed filesystem resolution — so nothing failed a claimed job, and no catch is taken. **Master-dev ruling (recorded):** the held candidate's "second instance across two distinct PRs" condition was aimed at preventing same-arc double-counting; a second instance in a DISTINCT ARC, of a DIFFERENT artifact class (a code-review-gate coverage gap vs. a hand-off INPUTS-block citation), satisfies it. Codified.

### 4 — Catch 66 (NEW, Class A — change-site contradiction): fabricated table qualifier

**Catch 66.** In this arc's CF-68 ratification prompt, a Phase 1 disposition (D3) asserted `accounts.lastDailyAttempted` as the ratified streak-state carrier — synthesizing an unverified TABLE qualifier (`accounts.`) onto a bare field name (`lastDailyAttempted`) and stating it as a ratified disposition BOUND FOR THIS append-only entry. Refuted at the change site against db/schema.ts: the column is `player_saves.last_daily_attempted` (`playerSaves`, db/schema.ts); `accounts` is identity-only. Surfaced at the halt gate and corrected before any append (D3, § 2, records `player_saves`). **Rule 29 cutting both ways, one arc after codification (this is the disposition, not an aside):** the SAME arc produced a NO-catch and a CATCH. The `useRun.ts` line drift took NO catch because the INPUTS block disclaimed line-number authority and no artifact claimed that job (Rule 29, § 3, Instance 2). This table qualifier takes a CATCH because NOTHING disclaimed it — it was asserted as ratified fact headed for canon, so the claim owned the job and failed it. Same lineage as Catch 64 (decision-log.md 2026-07-18 § "CF-77 CLOSED — Phase 2 PR2 …").

### 5 — Rule 30 (NEW): no unverified code-site qualifier

**Rule 30.** Never emit a qualified code-site citation — package, module, table, namespace, or any other qualifier — that was not returned verbatim from a tool. Qualify from ground truth or ask. Binds master-dev output at every boundary, not only hand-off INPUTS blocks. **Instances:** Catch 64 — a package prefix (`packages/sim/src/`) synthesized onto a bare `schemas.ts` (CF-77 arc, decision-log.md 2026-07-18 § "CF-77 CLOSED — Phase 2 PR2 …") — and Catch 66 — a table prefix (`accounts.`) onto a bare `lastDailyAttempted` (this arc, § 4). **Load-bearing finding (recorded):** the post-Catch-64 discipline was worded narrowly — "never infer a package prefix" — and that NARROWNESS is exactly why it did not fire on a TABLE prefix. The second instance proves under-generalization; Rule 30 SUPERSEDES the narrow wording, generalizing to any qualifier from any namespace. Second instance across two distinct arcs — codification gate met.

### 6 — Drift 46 (NEW, Topic 2): repeat of a one-arc-old discipline

**Drift 46.** Master-dev violated a discipline codified one arc earlier: the fabricated-qualifier practice flagged at Catch 64 / Drift 45 (decision-log.md 2026-07-18 § "CF-77 CLOSED — Phase 2 PR2 …") recurred here as the `accounts.` table prefix. Pairs to Catch 66 (§ 4) exactly as Drift 45 paired to Catch 64 — the change-site contradiction is the catch, the process-discipline violation is the drift. The aggravating fact — that it is a REPEAT of a discipline one arc old — is the reason it is recorded rather than absorbed.

### 7 — Citation-discipline AMENDMENT (extends an existing convention — no ordinal)

The standing never-cite-line-numbers convention (decision-log cross-references by date + section header only; append-only growth shifts line numbers) is EXTENDED from decision-log citations to CODE-SITE citations carried across a session boundary or appearing in a hand-off INPUTS block: cite file + symbol/anchor; a line number is a convenience annotation, explicitly marked drift-prone. This covers the LINE-NUMBER axis, distinct from Rule 30's QUALIFIER axis (§ 5). Evidence: the `useRun.ts` `createRun` line reference carried into this session drifted between sessions and was corrected against the filesystem this session. PR-A and PR-B will themselves move the sites they cite, so the amendment binds immediately. It EXTENDS an existing convention and does NOT increment the rule counter.

### 8 — CF-81 OPENED (NEW): shared-board daily seed

Whether the M2 daily adopts the server-issued daily seed (`DailyContractResponse.seed`, FNV-1a date-stable) as the run RNG basis — the precondition for everyone-same-board. UNSCOPED; own Phase 1. Explicitly COUPLED to CF-56 (shop-gen RNG basis) and ADJACENT to CF-64 (frozen-corpus regeneration-reproducibility drift). Ratified boundary: this is OUT of CF-68 because an RNG-basis change smuggled into a wiring PR is the exact hazard the CF-58 corpus-regeneration protocol exists to prevent (decision-log.md 2026-07-11 § "CF 58 CLOSED …"). CF number walked from canon: highest existing = CF-80 (decision-log.md 2026-07-17 § "CF-77 Phase 1 RATIFIED …", CF-79 + CF-80 opened) → CF-81.

### 9 — CF-79 AMENDED: daily-credit non-reconciliation added to scope

CF-79 (replay-validate) gains a second residual alongside reload-durability: daily-credit non-reconciliation. The participation row lands on the FIRST accepted push of a daily run (partial delivery still credits the day), but a run whose pushes ALL fail at the bounded-retry cap (`MAX_PUSH_ATTEMPTS`, run/RunContext.tsx) or are discarded on affirmative sign-out loses the day with NO server-side recovery — trophies reconcile (server-wins + eventual replay), daily-streak does not. A separate run-end daily-completion push was CONSIDERED and REJECTED: R5 established the per-round fold (decision-log.md 2026-07-18 § "CF-77 Phase 2 PR2 — PHASE 1 RATIFIED …"), and a second delivery path re-opens the exact push/auth boundary both Codex rounds on PR \#51 worked. CF-79 stays OPEN (no open-CF delta — amending scope, not opening/closing).

### 10 — PLAN-\*.md disposition, RATIFIED for deletion

All four untracked working-tree plans — `PLAN-cf58-trigger-chance.md`, `PLAN-cf59-gold-economy.md`, `PLAN-cf60-adjacency-signal.md`, `PLAN-client-combat-parity.md` — verified this session as FULLY EXECUTED with their CFs CLOSED (CF 58 → decision-log.md 2026-07-11 § "CF 58 CLOSED …"; CF 59 → 2026-07-09 § "CF 59 CLOSED …"; CF 60 → 2026-07-10 § "CF 60 CLOSED …"; CF 63 + CF 42 → 2026-07-08 § "CF 63 CLOSED … CF 42 CLOSED …"), against both live code and the log. Ratified for DELETION — the canonical record lives in the closing entries + shipped code; the untracked plans hold nothing the log does not. A `.gitignore` glob `PLAN-*.md` is ALSO ratified, with the rationale that the untracked-file reminder demonstrably FAILED to prompt disposition (the four sat `??` in the tree across the entire CF-77 arc). Deletion + the `.gitignore` line land in a SEPARATE housekeeping commit (this docs entry stays docs-only).

### 11 — Counter-integrity riders — two pre-existing anomalies, recorded canonically for the first time

Both were grep-verified against this file this session with passing positive controls, and both had previously lived only in the position topic file — a log entry is the only fix. Recorded in the grep-safe hyphenated form per decision-log.md 2026-07-16 § "Rule 26 codified (grep-safe ordinal references)": a literal spaced token would let a highest-ordinal walk read the anomaly as a live entry, and for the vacancy would additionally mask it under a distinct-count `grep -u`. Neither anomaly moves the tip counter (§ 12): under the ledger's highest-ordinal-reached convention a vacancy cannot lower, and a collision cannot raise, the highest ordinal reached.

(a) **Vacant rule ordinal.** The rules ledger has no entry at `rule-ordinal-9`; it was never minted (positive controls: `rule-ordinal-8` and `rule-ordinal-10` resolve to real rules). Highest rule ordinal reached is unchanged; distinct rules = highest rule ordinal reached − 1. The 9-slot is permanently vacant and will never be assigned.

(b) **Collided catch ordinal.** The catch ledger assigns `catch-ordinal-9` to TWO distinct catches — the predicate-vs-name lineage catch (decision-log.md 2026-05-07 § "CF 31 closed …") and the opponentGhostId type-signature-drift catch (decision-log.md 2026-05-13 § "M1.5a PR 1 closed …", whose counter line reads "8 → 10"). Highest catch ordinal reached is unchanged; distinct catches = highest catch ordinal reached + 1.

### 12 — Counter

Baseline (tip `e7c3a34`, decision-log.md 2026-07-18 § "CF-77 CLOSED — Phase 2 PR2 …") 65/28/9/45/46 → **66/30/9/46/47**. Delta: catches **+1** (Catch 66 — Class A change-site contradiction, fabricated `accounts.` table qualifier); rules **+2** (Rule 29 — no-artifact-claimed gaps take no catch; Rule 30 — no unverified code-site qualifier); patterns **+0**; drifts **+1** (Drift 46 — Topic 2, repeat of the one-arc-old fabricated-qualifier discipline); open-CFs **+1** (CF-81 OPENED; zero closures — CF-68 selected but already open, CF-79 amended but stays open). The citation-discipline amendment (§ 7) extends an existing convention (no ordinal). The two counter-integrity riders (§ 11) are DOCUMENTARY — under the highest-ordinal-reached convention the phantom `rule-ordinal-9` does not lower the highest rule ordinal reached (now 30) and the double-assigned `catch-ordinal-9` does not raise the highest catch ordinal reached (now 66), so neither field moves; the § 11 offsets (distinct rules = 30 − 1 = 29; distinct catches = 66 + 1 = 67) hold against the post-entry highest-reached ordinals.
